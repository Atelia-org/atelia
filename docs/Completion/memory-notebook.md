...
- `Error` — 流中错误
- `TokenUsage` — 累计统计；由 `message_start` / `message_delta` 中的 usage 字段逐步累积，流结束时作为 `CompletionChunk` 流中的最后一个 usage chunk 上浮
...
5. 上层通过 `CompletionChunkAggregation.AggregateAsync(...)` 逐块消费，组装成 `AggregatedAction`；`Agent.Core` 再按需包装成 `ActionEntry`
```
