...
对 `MethodToolWrapper` 这类 schema-bound tool 来说，错误（解析根本失败）会直接转成失败的 `ToolExecuteResult`，警告则作为补充信息附着在执行结果文本中；这些解析细节不会进入 history。
...
- `CompletionHttpTransportFactory` 负责创建 live / record / replay 用 `HttpClient`，并统一规范化 `BaseAddress`（absolute、无 query/fragment、路径前缀补齐尾随 `/`）
- provider 继续只依赖普通 `HttpClient`
- provider 构造器只做 fail-fast 校验：要求调用方传入已配置、可安全拼相对 URI 的 `HttpClient.BaseAddress`，但不会偷偷改写调用方的 `HttpClient`
...
├─ Gemini/
│  ├─ GeminiClient.cs               ICompletionClient 实现
│  ├─ GeminiApiModels.cs            generateContent 请求/响应 DTO
│  ├─ GeminiMessageConverter.cs     IHistoryMessage[] → Gemini contents[]
│  ├─ GeminiStreamParser.cs         SSE content parts parser
│  ├─ GeminiReplayBlock.cs          Gemini provider-native replay block
