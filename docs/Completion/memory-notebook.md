...
`ICompletionClient.StreamCompletionAsync(request, ct)` 对外返回一个完整的 `AggregatedAction`。
...
| `message_stop` / `[DONE]` | 流结束（client 在此后一次性写入 `Usage`） | — |
...
| `[DONE]` | 流结束；若仍有未刷出的工具调用则补刷，最后写入 `Usage` | — |
...
2. `await ICompletionClient.StreamCompletionAsync(request)`
...
4. 逐行 JsonNode.Parse → 分发到对应 provider 的 StreamParser
   ...
   └─ usage delta → 更新 usage（流结束时 `AppendTokenUsage(...)`）
