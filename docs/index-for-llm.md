...
> **更新**: 2025-07-25 (类型索引重构 - 全部完成)
> **状态**: ✅ 索引重构完成 (19/19 文档已重构)
...
(285行) 核心数据类型、枚举、标识符
- `GuidEncoder` - 统一GUID编码工具类，当前使用Base64编码(22字符)，支持未来切换到Base4096-CJK(11字符)
- `GuidEncodingType` - GUID编码类型枚举，支持Base64、Base4096-CJK和旧格式检测
- `NodeId` - 认知节点的唯一标识符，使用GuidEncoder.ToBase64String生成，支持字符串格式和相等性比较
- `RelationId` - 关系标识符，使用GuidEncoder.ToBase64String生成，用于唯一标识节点间的语义关系
...
- **重要决策**:
  ...
  - **🆔 GUID编码优化 (v1.2)**: 识别当前12位截取方案的冲突风险，设计了三种LLM友好的编码方案：Base64(22字符,已实施)、Base4096-CJK(11字符,开发中)、智能短名称(4-8字符动态,候选方案)，已实施统一的GuidEncoder工具类
