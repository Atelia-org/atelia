...
- `ICacheStrategy<TKey, TValue>` - 泛型缓存策略接口，定义缓存的基本操作规范
- `CacheStatistics` - 缓存统计信息记录，提供缓存命中率、请求数等统计数据
- `INodeCacheService` - 节点缓存服务接口，提供节点数据的多层缓存管理
...
- `NodeService` - 节点服务示例类，展示缓存服务的集成和使用模式
