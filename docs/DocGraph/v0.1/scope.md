...
3. **文档图闭包构建**
   ...
   - 递归追踪关系，构建完整的可达文档集合
   - 支持图结构（允许环），不强制树结构
   - **[设计决策] 循环引用不检测不报告**：核心目标是"收集信息"，环不影响这个目标，只需确保每个文档只被 visit 一次（类比：计算所得税，循环交易不影响针对每个个体的统计）
...
4. **双向链接验证**
   ...
   - **produce_by关系验证**：
     ...
     - 悬空引用汇总到报告
     - 缺失反向链接汇总到报告
   - **[设计决策] 单文档问题是 Warning 不是 Error**：单个文档的 frontmatter 解析失败或字段缺失，不应阻断对其他文档的收集
...
9. **汇总文档**
   ...
   - **[格式约定] 按文档分组，使用完整路径**：
     - 二级标题使用 workspace 相对路径（inline code 格式）
     - 示例：`## \`atelia/docs/DocGraph/v0.1/api.md\``
     - 好处：阅读者能直接找到目标文档，避免重名问题
   - **[格式约定] 使用子弹列表而非表格**：
     - 每个文档下的条目使用子弹列表
     - 好处1：节约 AI 阅读者的 Token 消耗（同一文档多条目不重复路径）
     - 好处2：属性灵活，额外信息作为子列表元素呈现
...
4. **仅做最小路径处理**
   - 必须规范化相对路径`../`（消解`.`/`..`）
   - 必须检查路径越界（确保在workspace内）
   - 统一使用`/`作为路径分隔符
   - 不做高级路径推断（如符号链接解析、网络路径等）
