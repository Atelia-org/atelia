...
public sealed record ActionEntry(
    ...
    //     在 GetFlattenedText() 中变为 "ab"，丢失文本被工具调用打断的边界信息
...
foreach (var msg in context) {
    if (msg is IActionMessage action) {
        // 用 action.GetFlattenedText() + action.ToolCalls，自动得到过滤后的视图
    }
}

// 新 converter（Anthropic Messages / OpenAI Responses）
// 注意：IActionMessage 现在直接拥有 Blocks，不再有两层接口区分
foreach (var msg in context) {
    if (msg is IActionMessage action) {
...
**改动范围**：
- 将 `IRichActionMessage` 改名为 `IActionMessage`，删除旧 `IActionMessage`（旧 `Content` 属性 → `GetFlattenedText()` 方法）；`Blocks` 直接成为 `IActionMessage` 成员（在 **Completion.Abstractions**）
...
- Converter 端识别 `IActionMessage.Blocks`（路径打通，但 Blocks 中暂无 ThinkingBlock）
...
- `AnthropicMessageConverter` 在 `IActionMessage` 路径下识别 `ActionBlock.Thinking`，反序列化 OpaquePayload 为 native content block
- 完整测试矩阵（见 §8）
...
### D9：`IActionMessage` 与 `ActionBlock` 为何最终上提到 `Completion.Abstractions`？

**理由**（重构后修订）：真正需要被 provider converter 直接消费的是“assistant message 的富内容结构”，也就是 `IActionMessage（含 Blocks）+ ActionBlock`。它们是 Completion 与 Agent.Core 之间的公共语言，而不是 Agent 私有 history 机制。若把它们留在 `Agent.Core.History`，就会迫使 `Completion -> Agent.Core`，依赖方向反了。最终采纳：

- `IActionMessage`（原 `IRichActionMessage` 改名）放在 `Completion.Abstractions`
- `ActionBlock` / `ActionBlockKind` 放在 `Completion.Abstractions`
- `CompletionDescriptor` 放在 `Completion.Abstractions`
- `ProjectedActionMessage` / `ContextProjectionOptions` / `ProjectedInvocationContext` 继续留在 `Agent.Core`
