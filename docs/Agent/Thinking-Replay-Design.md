...
| 层 | 职责 | 主要类型 |
|----|------|----------|
| **真相层** | 忠实保存 assistant 输出的 ordered blocks，不丢任何 replay-grade 信息 | `ActionEntry.Message.Blocks` / `ActionBlock` 子类型 |
...
1. **唯一事实源**：`AgentState.RecentHistory` 中的 `ActionEntry.Message.Blocks` 是 thinking 内容的**唯一权威存储**。不存在并行 sidecar。
...
**迁移策略**：
- `ActionEntry` 是 Agent 历史层 envelope，只保存 `ActionMessage Message` 与 `CompletionDescriptor Invocation`。
- lossy derived view（`GetFlattenedText()` / `ToolCalls`）位于纯消息体 `ActionMessage` 上；provider converter 与上下文投影只消费 `ActionMessage`。
- 旧构造形式 `new ActionEntry(content, toolCalls, invocation)` 不保留兼容层，调用点直接构造 `ActionMessage`。

### 3.3 ActionMessage 与投影消息

**依赖方向声明（PR 2.5 再修订）**：PR 3 暴露出一个关键事实：`ActionBlock.Thinking` 的 `Origin` 需要引用 `CompletionDescriptor`，而 `ActionBlock` 已经在 PR 2 上提到 `Completion.Abstractions`。这说明 `CompletionDescriptor` 也不是 Agent 私有 policy，而是 **provider-neutral 的 invocation identity schema**，应与 `CompletionRequest` / `CompletionResult` / `ActionMessage` / `ActionBlock` 同层放在 `Completion.Abstractions`。真正的 Agent 私有语义是 `TurnAnalyzer`、`ContextProjectionOptions`、`ProjectedInvocationContext` 等。最终依赖方向保持为 `Agent.Core -> Completion -> Completion.Abstractions`，避免 `Completion -> Agent.Core` 的反向引用。

```csharp
namespace Atelia.Completion.Abstractions;

/// <summary>
/// Action 消息 DTO。位于 Completion.Abstractions，
/// 表达 provider-neutral 的 assistant 内容结构。
/// Blocks 是真相源；GetFlattenedText() 是 lossy derived view。
/// </summary>
...
/// <summary>
/// 投影层产出纯 ActionMessage。Blocks 已根据投影规则过滤（旧 Turn 的 ThinkingBlock 已被剥离）。
/// </summary>
private static ActionMessage ProjectActionEntry(ActionEntry actionEntry, ContextProjectionOptions options, ...) {
    var projectedBlocks = ProjectActionBlocks(actionEntry, options, ...);
...
foreach (var msg in context) {
    if (msg is ActionMessage action) {
        EncodeBlocks(action.Blocks); // 含 ThinkingBlock 时回灌
    }
}
```
...
**目标**：把 action 消息体升级为 Blocks，但语义保持不变。
...
**改动范围**：
- 删除富 action 接口层，保留具体 `ActionMessage : IHistoryMessage`；旧 `Content` 属性 → `GetFlattenedText()` 方法，`Blocks` 是 `ActionMessage` 的唯一真相源（在 **Completion.Abstractions**）
- 投影层直接产出 `ActionMessage`，不再新增投影专用 action record
...
- Converter 端识别 `ActionMessage.Blocks`（路径打通，但 Blocks 中暂无 ThinkingBlock）
...
**目标**：让 thinking 内容能从 Anthropic 流进入到 `ActionEntry.Message.Blocks`。

**改动范围**：
- `ThinkingChunk` record
- `CompletionAggregator.AppendThinking` 处理 Thinking case，构造 `ActionBlock.Thinking`
- `AnthropicStreamParser`：解析 thinking content_block 的 start/delta/stop 事件，聚合为 OpaquePayload
- 端到端流测试：Anthropic 模型返回 thinking 后，`ActionEntry.Message.Blocks` 中能看到 `ActionBlock.Thinking`
...
- `AnthropicMessageConverter` 在 `ActionMessage` 路径下识别 `ActionBlock.Thinking`，反序列化 OpaquePayload 为 native content block
- 完整测试矩阵（见 §8）
...
- Recap 生成器：处理 ActionEntry 时按 `GetFlattenedText()` 派生视图工作（不解释 ThinkingBlock，不破坏其存在）
...
| # | 场景 | 期望 |
|---|------|------|
| T1 | 通过 `ActionMessage` 创建 `ActionEntry` | `Message.Blocks` 保留传入顺序 |
| T2 | `ActionMessage` 直接通过 Blocks 构造 | `GetFlattenedText()` 为 text 块拼接，`ToolCalls` 为 toolcall 块 list |
| T3 | 含 Thinking 的 Blocks | `GetFlattenedText()` 不含 thinking 内容，`ToolCalls` 不含 thinking |
| T4 | 空 Blocks | `GetFlattenedText() == ""`，`ToolCalls == []` |
...
| A2 | Converter 编码 `ActionMessage` 含 ThinkingBlock | request body 中 content 数组含原样 thinking block |
...
- `ActionEntry.Message.Blocks + OpaquePayload` 已能 lossless 表达所需事实
...
### D9：`ActionMessage` 与 `ActionBlock` 为何最终上提到 `Completion.Abstractions`？

**理由**（重构后修订）：真正需要被 provider converter 直接消费的是“assistant message 的富内容结构”，也就是 `ActionMessage（含 Blocks）+ ActionBlock`。它们是 Completion 与 Agent.Core 之间的公共语言，而不是 Agent 私有 history 机制。若把它们留在 `Agent.Core.History`，就会迫使 `Completion -> Agent.Core`，依赖方向反了。最终采纳：

- `ActionMessage` 放在 `Completion.Abstractions`，作为实现 `IHistoryMessage` 的具体 action 消息体
- `ActionBlock` / `ActionBlockKind` 放在 `Completion.Abstractions`
- `CompletionDescriptor` 放在 `Completion.Abstractions`
- `ContextProjectionOptions` / `ProjectedInvocationContext` 继续留在 `Agent.Core`

这样抽象层只定义 provider-neutral message schema，Agent.Core 则负责 Turn / Invocation / projection policy。
...
### D11：`GetFlattenedText()` 为何用 `string.Concat` 而非 `string.Join('\n', ...)`？

**理由**（GPT5 R2 提出 + Claude R3 强化）：Anthropic / OpenAI / Gemini 的流式协议本身不保证 text block 之间存在隐含换行——块边界是 protocol-level 划分，不是排版语义。`GetFlattenedText()` 若强加 `\n` 反而会注入虚假信息，影响下游消费者（如 Recap、UI）的语义判断。换行语义应由具体 provider converter 在编码时按需添加。
