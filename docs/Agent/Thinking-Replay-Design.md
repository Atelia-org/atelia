...
/// <summary>
/// 富接口：能识别 Blocks 的 converter 路径。位于 Completion.Abstractions，
/// 只表达 provider-neutral 的 assistant 内容结构，不承载 Agent 私有 Invocation 元信息。
/// </summary>
...
**改动范围**：
- 新增 `IRichActionMessage` 接口（在 **Completion.Abstractions**，作为 provider-neutral 公共语言）
...
### D9：`IRichActionMessage` 与 `ActionBlock` 为何最终上提到 `Completion.Abstractions`？

**理由**（PR 2 实施后修订）：真正需要被 provider converter 直接消费的是“assistant message 的富内容结构”，也就是 `IRichActionMessage + ActionBlock`。它们是 Completion 与 Agent.Core 之间的公共语言，而不是 Agent 私有 history 机制。若把它们留在 `Agent.Core.History`，就会迫使 `Completion -> Agent.Core`，依赖方向反了。最终采纳：

- `IRichActionMessage` 放在 `Completion.Abstractions`
