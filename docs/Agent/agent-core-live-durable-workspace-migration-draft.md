...
> - 阶段 C 已继续向 durable truth 推进一步：repo-backed live path 下，`pending notifications` 的 append / drain、`ReplacePrefixWithRecap(...)`，以及 `AppendAction` / `AppendObservation` / `AppendToolResults` / `InjectActionContent` 这些 recent-history 主链入口，现已开始围绕 `AgentWorkspaceSession` authoritative mutation + targeted recent-history/cache reload 工作；其中 recap / drain 走 `LoadStateSnapshot -> ApplySnapshot`，notification append 走 targeted pending-cache refresh，而 recent-history append/injection 主链当前采用“入口前后 recent-history reload”的有界收口形态。
