...
> - 阶段 C 已继续向 durable truth 推进一步：repo-backed live path 下，`pending notifications` 的 append / drain，以及 `ReplacePrefixWithRecap(...)` 这两类已切开的 mutation path，已开始改走 `AgentWorkspaceSession` authoritative mutation；其中 recap / drain 走 `LoadStateSnapshot -> ApplySnapshot`，notification append 先收成 targeted pending-cache refresh；append-heavy history 主链暂仍保持现状。
