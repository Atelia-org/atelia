...
工具内部负责验证前置条件、写 StateJournal、返回结构化结果。当前主线的 `MethodToolWrapper` 已支持单输入对象 DTO 与递归 schema；首版仍建议把 GM 工具拆小，优先追求审计性与可测试性，而不是一把塞入过宽 payload。
...
- 首版真实 GM 的工具集合包含 `gm_create_location` / `gm_link_locations` / `gm_move_actor`，以及 Phase 3 的 `gm_create_item` / `gm_create_npc` / `gm_add_interaction` / `gm_set_visibility`。GM 可以创建少量可见物品、NPC 和 affordance，但仍不创建复杂规则。
...
- GM 工具集统一使用 `gm_move_actor`，真实多主体 GM 不再只能移动终端玩家，可以裁决 LLM Player 的探索、移动、分头行动和停留；终端玩家同样通过 `gm_move_actor(player, ...)` 移动。
...
| Tool 参数复杂 | `MethodToolWrapper` 虽支持对象/数组，但仍受声明器约束，过宽 payload 会拖高 prompt 与验证复杂度 | 首版继续拆小工具；超出声明器约束时再手写 `ITool` 或局部自定义解析 |
...
| `gm_move_actor` | 移动指定 Actor | actor 与目标 location 都必须存在 |
...
2. 新增 `GmWorldEditService`，先提供 `gm_create_location` / `gm_link_locations` / `gm_move_actor`。
