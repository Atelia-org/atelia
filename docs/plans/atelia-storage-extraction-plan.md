...
> 状态：已完成三名独立 Reviewer 的两轮审视并收敛，待后续实施。日期：2026-09-13。
> 本轮只编写计划；尚未迁移源码、创建远端、安装工具、提交 Git 或发布包。
> 本文件是迁移任务的唯一实施计划。所有新路径、脚本和验收命令除明确注明“已有”外，均为待实现产物。
...
本轮只读基准：atelia `fecbff4485e98da49570bb843683ad815cde3c5e`；durable-graph `4f74ee5662b54b68916bcd9d9759a708e802666b`。这是调查定位，不替代 P0 的实施基准。核对发现：
...
各阶段开始先确认上阶段退出条件；本轮未执行以下任何阶段。主线程拥有跨仓合同、共享文件集成及最终验收责任。受限子任务可并行编辑不重叠文件，但同一工作树中的 .NET 构建、测试、pack 串行运行。

| 阶段 | 可验收结果 | 当前状态 |
| --- | --- | --- |
| P0 | 固定源清单、旧包与旧数据基准 | 未开始 |
| P1 | 新仓独立源码构建与测试 | 未开始 |
| P2 | 五包与独立公开 API 示例 | 未开始 |
| P3 | 两消费仓、双模式与跨版本旧数据验证 | 未开始 |
| P4 | 干净候选复验与本地交付 | 未开始 |
| P5 | 远端发布及正式获取验证 | 后置，不阻塞本地准备 |
...
下表为本轮读到的环境事实；P0 应重查，不把工具存在等同于认证或运行验证成功。

| 资源 | 当前核对 | 用户是否现在需要准备 | 用途/可替代方案 |
| --- | --- | --- | --- |
| 本机 Windows + Git + .NET 10 + PowerShell 7 | 已发现 Git、SDK 10.0.201、pwsh；未跑构建 | 不需要另备机器 | 足够开始 P0–P4 |
| Python / git-filter-repo | Python 命令存在；git-filter-repo 命令不存在 | 不需要手工学习或安装 | 实施时 Agent 在任务局部安装；失败可快照提取 |
| 新仓本地目录 | `E:\repos\Atelia-org\atelia-storage` 尚不存在 | 通常不需要手工创建 | Agent 可创建；已有同名目录时先核实，不覆盖 |
| 临时磁盘空间 | E 盘约 5.5 TB、C 盘约 26.5 GB 可用，本轮快照 | 暂不需要 | 实验根和隔离包缓存放 E 盘任务专属目录，避免填满 C 盘 |
| WSL Ubuntu | 已列出 Ubuntu；内部 SDK/pwsh/目录权限未验证 | 可选 | 建议未来使用 `~/atelia-storage-lab/`，不在 `/mnt/e` 冒充 Linux 文件系统测试 |
| 第二台电脑或全新 VM | 不需要 | 不需要 | 干净 clone + 独立缓存可做最低验收；现有 WSL/后续 CI 可扩展环境覆盖 |
| GitHub 仓库/组织权限 | 未核实账号权限；当前 shell 找不到 `gh` | P5 前需要归属、可见性和有权限账号 | 可由用户网页创建空仓，或 Agent 准备 CLI 后由用户登录；不用预建 README/初始提交 |
| nuget.org 账号 | 未核实 | 仅公开发布前需要 | 用户注册/登录/确认邮箱和 2FA；确定 PackageId 所有权，首次预发布无须 1.0 稳定承诺 |
| Trusted Publishing | 官方已支持；当前账号尚未配置、未测试 | 仅 P5 需要交互配置 | Agent 先准备 workflow 文件；用户在 nuget.org 配置精确 repo/workflow/包范围，通常不必长期保存 NuGet API key |
| 真实存档样本 | 可先由冻结旧包生成 | 暂不必提供私人业务目录 | 如已有必须保留的特殊存档，可指定可复制的样本路径；测试只作用于副本 |

不要求用户提前提供 NuGet API key、购买机器、搭建私有 feed、创建 VM 或学习 git-filter-repo。计划内的首次公开发布仍需用户拥有相应账号与权限。
