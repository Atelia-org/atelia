...
**决策**：增加一个极小的 `_isInitialized` 状态位，但仍不存储冗余的 `_isSuccess`。
- 🟢 优点：继续避免 `_isSuccess` / `_error` 双源状态不一致。
- 🟢 优点：可以把 `default(Result<T>)` 明确判为"未初始化失败态"，不再让引用类型的默认值偷偷伪装成 `Success(null)`。
- 🟢 优点：为 `[MemberNotNullWhen(true, nameof(Value))]` 提供运行时真实性支撑，避免 NRT 收窄与真实状态脱节。
- 🔴 代价：struct 多一个布尔字段；但换来的是契约自洽性与默认值安全性。

### 2.4 禁止 `Success(null)`

| 状态 | 含义 |
|:-----|:-----|
| `Success(value)` | 操作成功，产出一个非空值。 |
| `Failure(error)` | 操作失败，有错误原因。 |

**历史上**曾考虑过让 `Success(null)` 表达"查询返回空结果"。实际迭代后发现：

1. **与 `Failure(NotFound)` 语义重叠**，调用点需要判断"是 null 还是 Failure"，抑制了 Result 本身"二分分叉"的清晰度。
2. **与 NRT 不兼容**：只要保留 `Success(null)`，就不能给 `IsSuccess` 加 `[MemberNotNullWhen(true, nameof(Value))]`，调用方仍需 `result.Value!`，违背了"happy path 零 NRT 摩擦"的初衷。
3. **与 `Unwrap()` 不自洽**：`Unwrap()` 返回 `T`（非空），却允许实际返回 null。
4. **生产代码零依赖**：grep 仓库发现所有使用 `Success(null)` 的位置都在 Primitives 自身测试中。

理论渊源：Rust 用 `Result<T, E>` 和 `Option<T>` 两个正交抽象加以区分。Atelia 也采取同样立场：**Result 负责成败二分，不负责"成功但没有值"的表达**。如果未来需要显式建模该语义，应引入独立的 `Option<T>` / `Maybe<T>`，而不是重新放宽 `Result<T>`。
