...
### spec [S-MEMOTREE-STABLE-NODE-ID] Heading 节点必须有稳定 nodeId
...
节点摘要 MUST 记录它基于哪个正文版本生成，并允许判断当前摘要是否过期。
...
| `pinned` | `DurableDeque<string>` 或 `DurableOrderedDict<long, string>` | pinned / hotset 顺序 |
...
| `isPinned` | `bool` | 是否 pinned |
| `contentVersion` | `long` | 正文/结构版本 |
| `summaryVersion` | `long` | 摘要生成时基于的内容版本 |
