...
| [Store SQLite V2 logical-schema appendix candidate](../current/contracts/recap-grid-store-sqlite-v2.md) | tagged product source上Grid Store persistent pragma、5-table logical shape、canonical payload/bounds、locator proof与operator mapping的post-tag候选入口 | product source仍为`cd966fc7`；appendix/test evidence在tag之后形成；不属于immutable surface set 1，approval仍Defer | 审阅下一项Tier B Store candidate时；它是current contract appendix而非已批准evidence，不能因fingerprint/test通过自动freeze |
