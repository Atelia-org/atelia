...
| 审阅 Galatea root `config.json` | [Root config V2 current contract](current/contracts/galatea-root-config-v2.md)、[Galatea guide](../../prototypes/Galatea/README.md) | current product已hard-cut V2并新增per-user raw session provisioning；[V1 appendix](current/contracts/galatea-root-config-v1.md)仅保留prior approved history，旧tag不认证V2 delta |
