...
| Timeline、partition、HistoryLoad 与 branch reconcile | [HistoryLoad](current/derived-recap/history-load.md)、[HistoryTimeline code](../../prototypes/SessionJournal.HistoryTimeline/)、[HistoryTimeline tests](../../tests/SessionJournal.HistoryTimeline.Tests/) | [Durable authority](current/derived-recap/durable-target.md)、[Timeline schema 2 离线升级](../../prototypes/SessionJournal.Cli/README.md#timeline-schema-2-离线升级) |
...
| Grid Store、Control、Manager、Getter 与 Runtime | [Grid concepts](current/derived-recap/concepts.md)、[target design](work/active/derived-recap-grid-target-design.md) | [Store SQLite v4](current/contracts/recap-grid-store-sqlite-v4.md)与owning product/tests；[Timeline 单一行身份切片](../Galatea/timeline-row-identity-simplification-plan.md)已实现并通过本地验证，尚未部署，旧 Store v2/v3 证据只认证历史格式 |
| 审阅 Galatea root `config.json` | [Root config V9 current contract](current/contracts/galatea-root-config-v9.md)、[Galatea guide](../../prototypes/Galatea/README.md) | current product已hard-cut V9：required `homeDir` 为个人目录，delegates V3 删除全局 cwd；optional `serverAgentUserIds`决定后台驱动用户集合，每个user required `defaultConnectionId`，sibling connections保持无global default的V3 catalog；[V7](current/contracts/galatea-root-config-v7.md)及更早合同仅保留prior history，旧tag不认证V2–V9 delta |
...
- 计划入口（含为链接稳定保留原路径的closed plan）：
  ...
  记录 Timeline rows、analysis columns、按 CellSlot 定位的 immutable cells 与 Control/Store/Manager/Getter 的当前 Rule/Shape。
