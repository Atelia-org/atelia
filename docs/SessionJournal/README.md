...
| 审阅 Galatea root `config.json` | [Root config V3 current contract](current/contracts/galatea-root-config-v3.md)、[Galatea guide](../../prototypes/Galatea/README.md) | current product已hard-cut V3并新增required per-user `delegationStateDir`；[V2](current/contracts/galatea-root-config-v2.md)与[V1 appendix](current/contracts/galatea-root-config-v1.md)仅保留prior history，旧tag不认证V3 delta |
