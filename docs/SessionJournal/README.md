...
| `eadr.cadence.current` | NewPlanning 使用 HistoryLoad config V2；HistoryUnit count 只承担 structure/baseline/diagnostics，raw event count 只承担 resource safety/backpressure；[EADR concepts](event-addressed-derived-recap-concepts.md) | `concept`, `component-guide` · `current` | `implementation`；`cf3c77d524abdf24352400c221e0c42f0c9cb2fe`；config V2 resolver、HistoryLoad projector、plan evaluator + Resume/Restore tests；[DG1](session-journal-document-governance-dg1-pilot-report.md) | 修改 trigger、reserve、rolling interval、estimator identity 或 frozen-config boundary |
