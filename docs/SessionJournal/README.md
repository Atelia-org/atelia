...
| 审阅 Galatea root `config.json` | [Root config V5 current contract](current/contracts/galatea-root-config-v5.md)、[Galatea guide](../../prototypes/Galatea/README.md) | current product已hard-cut V5：required names保持不变，operator只提供`characterContextTemplate*`，binary以code-owned prefix/suffix物化finalized prompt；[V4](current/contracts/galatea-root-config-v4.md)、[V3](current/contracts/galatea-root-config-v3.md)、[V2](current/contracts/galatea-root-config-v2.md)与[V1 appendix](current/contracts/galatea-root-config-v1.md)仅保留prior history，旧tag不认证V2–V5 delta |
