...
| 审阅 Galatea root `config.json` | [Root config V4 current contract](current/contracts/galatea-root-config-v4.md)、[Galatea guide](../../prototypes/Galatea/README.md) | current product已hard-cut V4并新增required per-user `characterName`与startup prompt template materialization；[V3](current/contracts/galatea-root-config-v3.md)、[V2](current/contracts/galatea-root-config-v2.md)与[V1 appendix](current/contracts/galatea-root-config-v1.md)仅保留prior history，旧tag不认证V2/V3/V4 delta |
