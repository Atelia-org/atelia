...
| 生成或消费 SessionJournal offline validation report | [Offline validation report V3 tag-ready candidate](current/contracts/offline-validation-report-v3.md)、[surface set 6 addendum](evidence/contract-freeze-r2-approval-surface-set-6.md)、[Offline owner guide](../../prototypes/SessionJournal.Offline/README.md)、[activation runbook](operations/galatea-g2a-staging-acceptance.md#2-import-and-raw-baseline) | exact 25-field/current nested/7 phase/11 kind与read-only/publication/retry/privacy/resource boundary已获用户授权；unified gates/rebuild complete，final review/tag Pending；不属于immutable v5 tag |
...
| 审阅 approved public API / wire-format surface与remaining candidate | [Contract R2 approved surfaces and candidate map](current/contracts/session-journal-contract-r2.md) | immutable v1/v2/v3/v4/v5 tags分别锚定surface sets 1/2/3/4/5；[surface set 6 addendum](evidence/contract-freeze-r2-approval-surface-set-6.md)已完成gates/rebuild并tag-ready，final review/tag Pending；[plan](work/active/session-journal-contract-freeze-r2.md)保留其余Defer边界 |
...
- 当前 active plans：
  ...
  Offline validation report V3是其后的producer-only surface set 6 user-authorized candidate；fresh unified gates与
  provider-free rebuild已完成，candidate已tag-ready；final review与annotated v6 tag仍Pending。它不反向扩大surface set 5，
  也不把full audit/rebuild误写为bounded/content-free/physical-byte promise，或把public serialization metadata hard cut
  误写成CLR compatibility承诺。
