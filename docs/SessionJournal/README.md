...
| 修改 Cadence recent reserve或恢复丢失receipt | [Cadence set-reserve receipt candidate](current/contracts/cadence-set-reserve-receipt.md)、[CLI guide](../../prototypes/SessionJournal.Cli/README.md) | exact command-local status/detail/exit与fresh-inspect矩阵；post-v4-tag candidate、approval Defer；不要自动retry或把receipt当Cadence authority |
...
| 审阅 approved public API / wire-format surface与remaining candidate | [Contract R2 approved surfaces and candidate map](current/contracts/session-journal-contract-r2.md) | immutable v1/v2/v3/v4 tags分别锚定surface sets 1/2/3/4；[surface set 4 addendum](evidence/contract-freeze-r2-approval-surface-set-4.md)记录HistoryLoad exact窄scope、gates与tag object/target；[plan](work/active/session-journal-contract-freeze-r2.md)保留其余Defer边界 |
...
- 当前 active plans：
  ...
  independent review并由immutable v4 tag锚定；未列出的surface继续按Defer边界推进。
