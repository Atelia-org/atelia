...
| 修改 Cadence recent reserve或恢复丢失receipt | [Cadence set-reserve approved receipt contract](current/contracts/cadence-set-reserve-receipt.md)、[CLI guide](../../prototypes/SessionJournal.Cli/README.md) | exact command-local status/detail/exit与fresh-inspect矩阵已获surface set 5用户批准；unified gates/review/v5 tag Pending；不要自动retry或把receipt当Cadence authority |
...
- 当前 active plans：
  ...
  ledger/recovery，已获用户授权但unified gates、promotion review与v5 tag仍Pending；未列出的surface继续按Defer边界推进。
