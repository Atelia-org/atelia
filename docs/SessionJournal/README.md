...
| 修改 Cadence recent reserve或恢复丢失receipt | [Cadence set-reserve approved receipt contract](current/contracts/cadence-set-reserve-receipt.md)、[CLI guide](../../prototypes/SessionJournal.Cli/README.md) | exact command-local status/detail/exit与fresh-inspect矩阵已获surface set 5用户批准；unified gates通过/tag-ready，final ledger re-review/v5 tag Pending；不要自动retry或把receipt当Cadence authority |
...
| 审阅 approved public API / wire-format surface与remaining candidate | [Contract R2 approved surfaces and candidate map](current/contracts/session-journal-contract-r2.md) | immutable v1/v2/v3/v4 tags分别锚定surface sets 1/2/3/4；[surface set 5 addendum](evidence/contract-freeze-r2-approval-surface-set-5.md)记录Cadence receipt窄scope已授权且unified gates通过/tag-ready，final ledger re-review/tag Pending；[plan](work/active/session-journal-contract-freeze-r2.md)保留其余Defer边界 |
...
- 当前 active plans：
  ...
  ledger/recovery，已获用户授权且unified gates通过/tag-ready；promotion draft review已PASS，final ledger re-review与v5 tag
  仍Pending；未列出的surface继续按Defer边界推进。
