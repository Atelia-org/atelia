...
| 审阅 Galatea root `config.json` | [Root config V8 current contract](current/contracts/galatea-root-config-v8.md)、[Galatea guide](../../prototypes/Galatea/README.md) | current product已hard-cut V8：optional `serverAgentUserIds`决定后台驱动用户集合，每个user required `defaultConnectionId`，sibling connections保持无global default的V3 catalog；[V7](current/contracts/galatea-root-config-v7.md)及更早合同仅保留prior history，旧tag不认证V2–V8 delta |
