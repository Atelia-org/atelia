...
| 审阅 approved public API / wire-format surface与remaining candidate | [Contract R2 approved surfaces and candidate map](current/contracts/session-journal-contract-r2.md) | immutable v1/v2/v3 tags分别锚定surface sets 1/2/3；[surface set 4 addendum](evidence/contract-freeze-r2-approval-surface-set-4.md)记录HistoryLoad窄scope已授权且unified gates通过，review/tag Pending；[plan](work/active/session-journal-contract-freeze-r2.md)保留其余Defer边界 |
...
- 当前 active plans：
  ...
  v3 tag锚定；additive surface set 4只新增HistoryLoad V2 exact top-level/read-only contract，已获用户授权并通过
  unified gates，但independent review与v4 tag仍Pending；未列出的surface继续按Defer边界推进。
