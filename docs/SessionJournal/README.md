...
| 审阅 Galatea root `config.json` | [Root config V7 current contract](current/contracts/galatea-root-config-v7.md)、[Galatea guide](../../prototypes/Galatea/README.md) | current product已hard-cut V7：每个user required `defaultConnectionId`，sibling connections hard-cut无global default的V3 catalog；[V6](current/contracts/galatea-root-config-v6.md)及更早合同仅保留prior history，旧tag不认证V2–V7 delta |
