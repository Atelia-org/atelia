...
| 调查 durable-file primitive 收敛，或在 D0/D1 Go 后实施 conditional kernel | [S4-D DurableIO 安全收敛方案](work/active/session-journal-durable-io-safety-convergence.md) | 先锁四 owner threat/settlement matrix；public result family 当前 Retain |
...
- 当前 active plans：
  ...
  [S4-D DurableIO 安全收敛方案](work/active/session-journal-durable-io-safety-convergence.md)
  记录四个durable owner的共同kernel资格、停止门，以及为何public result family当前不合并。
