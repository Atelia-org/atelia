...
| D09 | `<repo>/control/recap-grid/v1/refs/<ref>/timelines/<timeline>/{control.json,lifetime.lock,writer.lock}` | WP-02 complete的单一Control authority；bounded canonical whole-state、双lock、exact Timeline binding；不在可reset的`derived` root | `SessionJournal.RecapGrid.Control/ControlRuntime.cs`、`ControlDurableFiles.cs` | Keep | L05/L11/L12 | WP-02/07A/07B/08 | 两个Hosts只读该carrier且old config零reader；backup/export/temp永不参与normal discovery | Open — WP-02 carrier/canonical/CAS/admission/backup/restore/crash/public surface已通过两路independent GO与final serial gates；Hosts/cutover仍未实施 |
| D10 | `<repo>/derived/recap-grid/v1/{grid.sqlite,lifetime.lock}` | WP-03 complete的唯一Grid artifact/ref authority；SQLite canonical BLOB + validated locators/members/FK；Grid-only reset | `SessionJournal.RecapGrid.Store/SchemaV1.sql`、`StoreRuntime.cs`、`StoreMaintenance.cs` | Keep | L06/L07 | WP-03/04/05/07A/08 | Manager/Getter/Hosts只经public Store API消费；normal不扫描temp/orphan/old roots，reset不触碰raw/Timeline/Control | Open — WP-03 Store、WP-04 Manager、WP-05 Getter与WP-07A operator slice均complete；WP-07A CLI只经public Manager/Getter/Store maintenance API消费，init按domain显式创建且stable store-only命令不变。Hosts/cutover仍属WP-07B/08；current production未切换 |
...
| L14 | `RecapExecutionLane*`、`RecapRuntimeGroup*`、`BoundRecapBlockMaintainer` | Runtime project；Galatea/CLI composition | family/lane affinity、leader/follower、parallel cap、logging/cache与semantic identity分离 | unique real `IRecapCellBatchExecutor` | Rewrite | WP-06 | scheduler/cache/cancel/drain/renderer tests green；Manager无第二scheduler | Open — WP-06 runtime slice complete：`SessionJournal.RecapGrid.Runtime`实现exact lazy route、不持lane pre-admission + actual leader-start/terminal barrier、route-reference host cap、timeout/cancel/batch-fatal drain、explicit Owned/Borrowed lifetime与bounded operational telemetry；Runtime 56/56、Completion 471/471、public 2/2、Walking 18/18、solution 0W/0E且两路independent GO。Host composition与old owner删除仍属WP-07B/08 |
...
| L22 | Galatea `connections.json.recapMaintainerConnections`与CLI active `--connections`/`--connection` route | `GalateaConfig.cs:19-40`、`GalateaServices.cs:1272-1354`、`OnlineTurnCommand.cs:13-41,146-213` | exact allowlisted route、无default fallback、provider route不进durable identity；static every-column/implicit agent-route不保留 | Host family/runtime route registry | Retarget | WP-06/07B | 两Host dynamic column按family key解析；missing exact route拒绝；frozen completion registry保留 | Open — WP-07A candidate新增独立Hosting strict bounded route/connections reader、exact lazy resolver与bounded lazy call evidence，显式null semantic且无fallback，并锁Runtime-before-registry lifetime；Galatea与`run-online-turn`复用及old mapping删除仍属WP-07B/08 |
...
| T07 | Planner campaign/kernel/rebuild/lifecycle/config tests | budget、drain、missing-only、安全lifecycle重写；epoch/config mechanics删除 | Rewrite | WP-04/06/07 | fake/real batch与lifecycle/operator matrices green | Open — WP-04 complete fake Manager 57/57已覆盖纯build算法、batch/drain、budget、restart、authority tail与mystery并取得两路independent GO；real executor、真实Idle/Observation/ToolResult lifecycle、operator与old test deletion仍Open |
