...
| L05 | old `DerivedRecap.Abstractions`: `RecapMaintenanceEpochInput`、`IRecapBlockMaintainer*`、group/call-control/registry | `RecapMaintenanceContracts.cs:9-260`；Planner kernel、Runtime、Host registries | `Updated`/`KeepUnchanged` distinction、lazy binding、runtime identity不进durable identity | RecapGrid.Abstractions + Manager batch seam | Rewrite | WP-02/04/06 | canonical definitions、`FrozenRowBatch`、closed per-item outcomes green；old namespace production zero | Open — WP-04 complete已交付Manager-owned provider-neutral batch seam和fake dense matrix并取得两路independent GO；WP-06 real executor与WP-08 old namespace deletion未完成 |
| L06 | v8 wire/layout：`DerivedRecapV8Contracts`、`DerivedRecapV8Codec`、`RecapEpoch*` wire types、`EventAddressFileNameCodec` | Store/Planner/CLI/tests | strict canonical codec、hash/bounds/fail-closed作为行为参考；epoch/layout本身不保留 | RecapGrid.Store canonical artifacts | Delete | WP-03/05 then WP-08 | new schema/strict/corruption/materialization tests green；old schema/type/path scan zero | Open — WP-03 new canonical/schema candidate已落地且不读v8；Getter与WP-08 old owner deletion仍未实施 |
| L07 | `DerivedRecapEpochStore`、repair/reseal/write authorities、`RecapDurableFileSystem` | Galatea/CLI、Planner、Store tests；`DerivedRecapEpochStore.cs:19-77` | immutable commit、atomic winner、crash safety、read bounds；Building/Published/repair语义不保留 | SQLite RecapGrid.Store | Delete | WP-03/04/05 then WP-08 | SQLite crash/contention/reset、missing-only resume、strict Getter green；old owner/tests removed | Open — WP-03 SQLite Store candidate已覆盖三transaction crash、contention与Grid-only reset；Manager/Getter/cutover尚未实施 |
...
| T04 | `DerivedRecapEpochStoreCandidateTests`、`DerivedRecapEpochCrashRecoveryTests` | immutable/crash/reset/strict read行为迁到SQLite/Grid；Building/Published mechanics不保留 | Rewrite | WP-03/05 | new Store crash/contention/corruption/Getter tests green | Open — WP-03 candidate已落三commit 9-case child crash、reset 2-case、contention/corruption/public fixtures；Getter slice待WP-05 |
