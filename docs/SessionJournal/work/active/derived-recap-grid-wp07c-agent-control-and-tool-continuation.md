...
- registration bundle与promotion分别是单次Control transaction。promotion先对current Timeline head执行
  `ExplicitCandidate`且`MaximumNewCalls=0`，只接受`Fulfilled(RecapGridPromotableProof)`；partial receipt、missing、stale
  均不得activate。proof不编码、不落盘。
- lifecycle trigger显式区分`PreObservation`、`ObservationAccepted`、`ToolResultObserved`。只有
  `PreObservation`允许Online reconcile/seal；后两者只做readiness，当前Observation/ToolResult必须保留在
...
- candidate CLI与Galatea的Prepared先exact bind frozen completion与frozen tool identity，但AgentControl binding保持lazy、对derived
  owners零触达；ToolContinuation先bind frozen tool profile，再bind current completion，最后才在需要时打开Online。current
  public/default composition、旧top-level `run-online-turn`与旧`recap`语义均未切换。
- dense fixture以真实`ToolSession`登记Family、`CulpritHypothesis`/`XSuspicion` definitions、full base与overlay recipe作为明确
  ...
  `candidate run-online-turn` Host内发出`recap_grid.control` promotion ToolCall；同一Host以`MaximumNewCalls=0`重证并提交
