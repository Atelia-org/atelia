...
[V2](galatea-root-config-v2.md)，新增required per-user raw SessionJournal provisioning policy。下文由surface set 2
tag锚定的root V1内容仅保留prior approved history；旧tag不认证V2 delta，V1 appendix与approval evidence不改写。
...
| Galatea root config | **Current product V2**：[current contract](galatea-root-config-v2.md) hard-cut exact `v:2`，新增required user-level `sessionProvisioning`；**prior Approved Stable V1**仍见[历史appendix](galatea-root-config-v1.md) | V2不由旧tag认证；V1 product source `8c450bf0` + integration evidence `6c5d3d50`及immutable surface-set-2 approval均只保留历史含义 |
...
hard-cut到未由旧tag认证的[root config V2](galatea-root-config-v2.md)；
