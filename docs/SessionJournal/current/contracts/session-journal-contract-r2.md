...
[V3](galatea-root-config-v3.md)，在V2 per-user raw SessionJournal provisioning policy基础上新增required
`delegationStateDir`。下文由surface set 2 tag锚定的root V1内容与后来的V2合同仅保留prior history；旧tag不认证
V2/V3 delta，历史appendix与approval evidence不改写。
...
| Galatea root config | **Current product V3**：[current contract](galatea-root-config-v3.md) hard-cut exact `v:3`，新增required user-level `delegationStateDir`；V2 `sessionProvisioning`合同与**prior Approved Stable V1**分别见[历史V2](galatea-root-config-v2.md)/[V1 appendix](galatea-root-config-v1.md) | V2/V3不由旧tag认证；V1 product source `8c450bf0` + integration evidence `6c5d3d50`及immutable surface-set-2 approval均只保留历史含义 |
...
[root config V3](galatea-root-config-v3.md)；
