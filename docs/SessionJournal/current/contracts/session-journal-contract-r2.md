...
| Galatea root config | **Current product V6**：[current contract](galatea-root-config-v6.md) hard-cut exact `v:6`，保留V5 code-owned prompt composition并新增required `characterMemoryStateDir`与total storage topology；当前配置包不open/create Character Memory state；V5 prompt ownership、V4 full-template、V3 delegation storage、V2 `sessionProvisioning`及**prior Approved Stable V1**分别见[历史V5](galatea-root-config-v5.md)/[历史V4](galatea-root-config-v4.md)/[历史V3](galatea-root-config-v3.md)/[历史V2](galatea-root-config-v2.md)/[V1 appendix](galatea-root-config-v1.md) | V2–V6不由旧tag认证；V1 product source `8c450bf0` + integration evidence `6c5d3d50`及immutable surface-set-2 approval均只保留历史含义 |
