...
| Galatea root config | **Current product V4**：[current contract](galatea-root-config-v4.md) hard-cut exact `v:4`，新增required `characterName`、template source字段与startup finalized prompt；V3 delegation storage、V2 `sessionProvisioning`及**prior Approved Stable V1**分别见[历史V3](galatea-root-config-v3.md)/[历史V2](galatea-root-config-v2.md)/[V1 appendix](galatea-root-config-v1.md) | V2/V3/V4不由旧tag认证；V1 product source `8c450bf0` + integration evidence `6c5d3d50`及immutable surface-set-2 approval均只保留历史含义 |
...
先后hard-cut到未由旧tag认证的[root config V2](galatea-root-config-v2.md)与
[root config V3](galatea-root-config-v3.md)，current又hard-cut到
[root config V4](galatea-root-config-v4.md)；
