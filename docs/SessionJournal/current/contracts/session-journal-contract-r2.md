...
状态：approved surface sets 1 + 2 + 3 + 4 anchored；surface set 5 user-authorized、unified gates/review/tag Pending；其余surface继续candidate/Defer  
...
surface set 5 production/test source：`4e1e80e6875a3a963bd90c3845250da261548730`；candidate docs：`6ed308f0268d8e337753252aad0d2ad4f5039eb8`  
...
ledger/recovery窄scope，但本轮unified gates、promotion review与annotated v5 tag仍Pending。除上述exact scope外的
...
result、generic outer envelope、Cadence durable V1或receipt atomicity。该授权不会反向扩大v4 tag；v5 gates/review/tag仍Pending。
...
- surface sets 1与2分别由immutable v1/v2 tags锚定；v2 tag exact object为`13111f3d`，dereferenced target为
  ...
  [surface set 4 addendum](../../evidence/contract-freeze-r2-approval-surface-set-4.md)。Surface set 5已获用户授权，但
  unified gates、promotion review与exact tag `session-journal-contract-r2-approved-surfaces-v5`均Pending；exact范围见
