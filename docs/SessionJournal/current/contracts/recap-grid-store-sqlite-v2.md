...
logical-schema scope；pre-tag gates已通过并由annotated v2 tag锚定。该批准不改变product、schema或accepted
...
non-promises界定；pre-tag unified gates与independent review已通过，annotated v2 tag已锚定`c4c6dd16`。当前
post-tag文档只记录closure，不反向移动或改变该tag。
