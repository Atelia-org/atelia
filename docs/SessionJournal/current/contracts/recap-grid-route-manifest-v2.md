...
`connections.json` 拥有 connection 的 provider、model、endpoint、credential locator、reasoning、cache 与
provider/client output setting。切换 `connectionId` 时，这些配置作为一个整体切换；route 不得再声明
`maximumOutputTokens` override。

Recap runtime 不拥有“输出请求预算”。它构造的 `CompletionRequest.MaxTokens` 恒为 `null`。若 provider wire
必须提供有限输出上限（当前 Anthropic client 即如此），该值由 selected connection 创建的 client 提供；
`openai-codex-responses` connection 则必须保持 `MaxTokens == null`，直到存在经过验证的 mapping。
...
V1 entry 的 `maximumOutputTokens` 被删除，不读取也不迁移。operator 必须停服并成对更新 current binary、
route manifest 与 referenced connections：

1. 把仍需保留的 provider/client output setting 写入 dedicated connection；
2. route 改为 `v:2` 并删除 `maximumOutputTokens`；
