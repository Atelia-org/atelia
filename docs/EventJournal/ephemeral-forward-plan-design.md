...
internal sealed class EphemeralForwardPlan {
    ...
    public required ulong PhysicalLayoutEpoch { get; init; }
}
```
...
BuildEphemeralForwardPlan(head):
    ...
    return Plan(root, head, eventCount, redirectsReverse, currentEpoch)
```
...
```text
Replay(plan):
    require plan.Epoch == journal.PhysicalLayoutEpoch
