...
`SegmentedFrameStore` 是 EventJournal 之下的轻量基础层。它不把 store 文件夹伪装成一个新的跨 segment RBF 文件，也不封装 RBF 的 frame append/read API；它只负责 RBF segment 文件的路径、创建、打开、轮转、lease、打开池和 active-tail 恢复编排。
...
打开 store 时，`SegmentedFrameStore` 扫描 `segments/`：
...
7. 若没有任何 segment，则创建 `SegmentNumber = 1` 作为 active segment。
...
读路径按地址中的 `SegmentNumber` 借出 reader lease；若目标是 active segment，store 必须与 active writer 协调并返回可安全读取的 lease，若目标是 closed segment，则通过 historical reader pool 返回 read-only lease：
...
2. 若大小已达到或超过 `SegmentSizeThreshold`，关闭并 sealed 当前 active segment。
...
轮转判断发生在写入前，因此单个 frame 可能使 segment 最终大小超过阈值；最大超出量约为一个 RBF frame 的最大大小。该简化被接受，以换取 `SegmentedFrameStore` 不需要预估下一 frame 大小，也不需要理解上层写入策略。RBF 仍保证单个 frame 不跨 segment。

推荐首轮 `SegmentSizeThreshold` 仍可从 64 GiB 起步，但这是 options 默认值，不影响路径和地址格式。

## 7. Reader Lease 与 Historical Pool

`OpenReader(segmentNumber)` 必须同时支持 active segment 与 closed historical segment。historical segment 以 read-only 方式按需打开并进入 pool；active segment reader 不进入 historical pool，必须与 active writer 生命周期协调。pool key 是 `SegmentNumber`，不是路径。

MVP 行为：

- active writer 独占 active segment 的写入权。
...
RBF 层负责单文件“寻找最后一个完整有效 frame / recoverable tail offset”。`SegmentedFrameStore` 只编排 active segment 的恢复：
...
因为 `SegmentedFrameStore` 不解释 frame tag，它不能判断 Event 是否可见，也不处理 orphan payload / orphan event。这些属于 EventJournal。

## 9. 作为独立 Project

该层可以作为独立 C# project 施工，例如 `src/SegmentedFrameStore` / `Atelia.SegmentedFrameStore`。它依赖 RBF 与基础 primitives，但不依赖 EventJournal、ChatSession 或 StateJournal。

首轮测试目标：

- `SegmentNumber` 到路径的 bit-split 映射，覆盖至少两个 bucket。
- 空 store 打开后创建 segment `1`。
