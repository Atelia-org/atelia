...
`segmentSlot` 仅用于校验、诊断或未来局部索引；文件名仍使用完整 `segmentNumber`，避免文件脱离目录后失去自描述性。
...
5. 布局版本、`SegmentBucketBits`、bucket name width、segment file name width 和后缀记录在 store manifest 中，并在 store 创建后保持不变。
6. 路径始终是可重建的派生信息，不写入 `EventAddress`。

该布局不需要运行时维护 segment range table。根据 `SegmentNumber` 可直接计算目标目录与文件路径；打开 store 时只需验证 manifest、active segment 和已存在 segment 文件的连续性与命名一致性。
