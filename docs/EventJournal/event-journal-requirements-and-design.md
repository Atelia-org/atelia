...
- `EventAddress.FrameAddress` 必须指向一个有效 `MetaFrame`，是 Event 的稳定身份。
...
`AddressHint` 是 32-bit smart-pointer hint，例如应用 event kind、粗粒度时间桶或染色标记。它属于 `EventAddress`，不属于通用 `FrameAddress`；payload part address list 默认保持无 hint，以避免大规模地址表无谓膨胀。
...
`FrameAddress` 保持 12 bytes，使 MetaFrame 中的大规模 payload part address list 不为 Event-level hint 付费。`EventAddress` 使用 16 bytes，额外 4 bytes 只出现在 Parent、branch head、reflog old/new head 等 Event 身份位置。
...
## 6. SegmentedFrameStore

### 6.1 定位

`SegmentedFrameStore` 是 EventJournal 之下的轻量基础层，完整职责边界与 API 设计见 [SegmentedFrameStore 设计基线](segmented-frame-store-design.md)。它不封装 RBF 的 frame append/read API，只负责 RBF segment 文件的生命周期管理，并通过 lease 向上层提供已打开的 `IRbfFile`。
...
路径布局（hex bit-split 公式、bucket 目录、文件命名规则、`SegmentNumber=0` 保留、无 manifest 策略）详见 [SegmentedFrameStore §3 Segment 路径布局](segmented-frame-store-design.md#3-segment-路径布局)。

路径始终是可重建的派生信息，不写入 `EventAddress`。该布局不需要运行时维护 segment range table——根据 `SegmentNumber` 可直接计算目标目录与文件路径。

### 6.3 Segment 生命周期

Segment 编号分配、轮转触发机制（size-only rotation）和 active/closed segment 状态管理详见 [SegmentedFrameStore §4 打开与发现](segmented-frame-store-design.md#4-打开与发现) 和 [§6 Active Writer 与轮转](segmented-frame-store-design.md#6-active-writer-与轮转)。
...
Reader lease、historical reader pool 的 lease 保护、LRU 淘汰、异常淘汰和 pool key（`SegmentNumber`）策略详见 [SegmentedFrameStore §7 Reader Lease 与 Historical Pool](segmented-frame-store-design.md#7-reader-lease-与-historical-pool)。
...
1. 通过 `SegmentedFrameStore.OpenReader(address.SegmentNumber)` 借出 MetaFrame 所在 segment 的 reader lease。
...
1. 由 `SegmentedFrameStore` 扫描 `segments/` 并验证 segment 命名、bucket 一致性和编号连续性。
...
### SFS-1：SegmentedFrameStore 独立层

范围：独立 C# project 形态的 `SegmentedFrameStore`。完整施工目标与验收标准见 [SegmentedFrameStore §9 作为独立 Project](segmented-frame-store-design.md#9-作为独立-project)。

### EJ-1：地址与 Frame IO Helper

范围：`FrameAddress`、`EventAddress`、Address wire format，以及基于 `SegmentedFrameStore` lease 组合 `SizedPtr Ticket + SegmentNumber` 的轻量 helper。
