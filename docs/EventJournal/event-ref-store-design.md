...
归档 ref 不是删除 move chain，而是在 `ref-op-log.rbf` 追加归档操作，并在 ref object 内追加 `Close` move。物理归档 segment 是后续 maintenance 能力，MVP 不在 active layout root 中放置 `.rbf.archived`。
...
```text
refs/objects/<ref-id-hex>/segments/<segment-number-8hex>.rbf
```

示例：

```text
refs/objects/0123456789abcdef/segments/00000001.rbf
refs/objects/0123456789abcdef/segments/00000002.rbf
```
...
- MVP 不使用 `.rbf.archived`；物理归档必须作为 `RbfSegmentStore` 通用 maintenance 能力另行设计。
...
Create refs/objects/<ref-id>/segments/00000001.rbf
...
- 物理压缩或移除 historical segment 是后续 maintenance 能力，不进入 MVP。
