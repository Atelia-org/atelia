...
> **状态**：Implemented Baseline（EventFrame v2 + identity/brotli/zlib 已落地）
> **日期**：2026-07-24
> **动机**：为 SessionJournal 等长寿命文本事件提供写入时可选压缩，同时保持上层 payload 语义稳定。
> **相关文档**：[EventFrame Parent Chain 设计基线](event-frame-parent-chain-design.md)、[EventJournal 功能需求与粗粒度设计基线](event-journal-requirements-and-design.md)、[SessionJournal 主干设计基线](../SessionJournal/archive/superseded/session-journal-trunk-design.md)

> **实现说明**：当前代码已原地升级 `EventFrameHeaderCodec` 到 v2；writer 支持 `identity`、BCL `brotli` 与 BCL `zlib`，`zstd-frame` 仍只保留 codec id。`ReadEventHeaderChecked` 只校验 stored frame/header，不解码 payload；`ReadEvent` 返回 logical payload。SessionJournal beta 默认使用 `zlib` 的保守自动压缩策略。
...
| 2 | brotli | 已实现/可选 | .NET BCL `System.IO.Compression.BrotliEncoder.TryCompress` / `BrotliDecoder.TryDecompress` 可用，零新增依赖、span-based 契合连续 payload；热写入等级不宜过高。 |
| 3 | zlib | 已实现/SessionJournal beta 默认 | .NET BCL `System.IO.Compression.ZLibStream` 可用，兼容性强；在当前中文+JSON payload benchmark 中比低等级 brotli 更适合作为 .NET 10 阶段默认压缩。 |
| 4..32767 | reserved | 保留 | Atelia 内置 codec |
| 32768..65535 | private/experimental | 实验 | 不应写入长期 journal，除非 manifest 固定说明 |

实现策略：

1. wire format 支持 metadata 与 identity codec，API 先稳定。
2. BCL 内置 codec 先落地 `brotli`（id = 2）与 `zlib`（id = 3），不引入 native 依赖即可打通压缩链路与验收测试。
3. `zstd-frame`（id = 1）保留为长期默认目标，待 .NET / 运行时依赖策略稳定后再实现。无论选择哪一个，codec id 必须固定，不能因实现库更换而变。

热压缩路径优先使用一次性 span API（如 `BrotliEncoder.TryCompress`）；没有合适 span API 的 BCL codec 可使用 stream API，但必须保持解码硬上界与自动收益门槛。
...
> **开销说明**：identity 路径不变，仍直接 `Append(tag, logicalPayload, tailMeta)`，零额外拷贝。压缩路径会先把 logical payload 压缩到临时 buffer，再作为 stored payload 交给 RBF，因此每次压缩写入多一次缓冲 + 拷贝。这是压缩的固有成本，不是零拷贝。
...
SessionJournal 的领域层无需感知 codec。它仍然把 canonical JSON bytes 传给 EventJournal，并从 `ReadEvent` 得到同一 logical bytes。当前 beta 阶段的 `SessionJournalEngine` 默认使用 `EventPayloadCodecPolicy.Zlib`，保守门槛为 `MinimumPayloadLength = 2048`、`MinimumSavingsBytes = 256`、`MinimumSavingsRatio = 0.05`；小 payload 或收益不足会自动写 identity。
...
3. **真实 codec 垂直切片**：先落地 BCL `brotli`（id = 2）与 `zlib`（id = 3），前者验证 span codec 路径，后者作为 SessionJournal beta 默认策略。`zstd-frame`（id = 1）待依赖策略确认后再补。codec id 必须按 registry 固定。
...
- .NET 10 当前 BCL 压缩 API 有 Brotli/ZLib/GZip，但 zstd 支持需要等待运行时依赖策略稳定。wire format 已预留 zstd id，不必让 API 绑定具体实现库。
