...
MVP 支持两种路径布局。layout 不写 manifest，而是由 store root 下的目录名区分：`buckets/` 表示分桶布局，`segments/` 表示轻量 flat 布局。segment 文件名始终使用完整 `SegmentNumber` 的 8 位小写 hex 表示。

### 3.1 Bucketed Layout

bucketed layout 使用按 `SegmentNumber` 高低位拆分的 hex 路径布局。每个 bucket 对应 1024 个 segment 编号槽位，bucket 目录名来自 `SegmentNumber` 去掉低 10 bit 后的高位。
...
4. `SegmentNumber = 0` 保留给 null/未设置编码，因此 `buckets/000000/00000000.rbf` 永远不存在；第一个 bucket 只有 1023 个有效 segment。
5. `segmentSlot` 仅用于校验、诊断或未来局部索引；文件名仍使用完整 `segmentNumber`，避免文件脱离目录后失去自描述性。
6. 路径始终是可重建的派生信息，不写入 `FrameAddress` 或 `EventAddress`。

### 3.2 Flat Layout

flat layout 不分桶，segment 文件直接位于 `segments/` 下：
...
1. segment 文件名为完整 `SegmentNumber` 的 8 位小写 hex 编码，后缀为 `.rbf`。
2. `SegmentNumber = 0` 保留，因此 `segments/00000000.rbf` 永远不存在。
3. `segments/` 下不得出现子目录。
4. 路径始终是可重建的派生信息，不写入 `FrameAddress` 或 `EventAddress`。

MVP 不引入 manifest 作为路径布局的权威来源。布局由 `buckets/` 或 `segments/` 目录名表达；打开 store 时通过目录扫描验证现有 segment 文件是否符合命名规则和连续性要求。若两个 layout root 同时存在，store 必须报告 ambiguous layout。
...
2. `OpenExisting` 要求 `storePath/buckets/` 或 `storePath/segments/` 恰好存在一个且至少包含一个合法 segment；它只打开和恢复现有 store，不创建 segment。若 store 为空或缺失，MUST 抛出异常。
3. `OpenOrCreate` 在 store 不存在或没有任何 segment 时创建 `SegmentNumber = 1`；若发现任何不合法 segment 文件或编号缺口，MUST 抛出异常，不得“修复后继续”。

打开已有 store 时，`RbfSegmentStore` 扫描 layout root：

1. bucketed layout 忽略不存在的 bucket 目录；flat layout 不允许子目录。
2. 拒绝无法按固定 hex 规则解析的 segment 文件。
3. 拒绝 `SegmentNumber = 0` 对应的 segment 文件。
4. bucketed layout 验证 segment 文件所在 bucket 与文件名中的完整 `SegmentNumber` 一致。
...
1. `NewStoreLayout` 只影响创建新 store；打开已有 store 时由目录名发现实际 layout。
2. `SegmentSizeThresholdBytes` 是 soft threshold，只在 `OpenActiveWriter()` 借出前检查；它 MUST 为正数且 4B 对齐。测试可以设置很小的值触发轮转。
3. `HistoricalReaderPoolCapacity` 表示最多保留多少个 idle historical reader；`0` 表示不缓存 idle reader，但仍必须尊重 live lease。
4. `CacheMode` 直接传给底层 `RbfFile.CreateNew` / `OpenExisting` / `OpenReadOnlyExisting`。
5. `RecoverActiveTailOnOpen` 控制 `OpenExisting` / `OpenOrCreate` 是否在打开现有 active segment 时执行 §8 的 active-tail recovery。`CreateNew` 创建的空 segment 不需要 recovery。
...
- `SegmentNumber` 到路径的 bucketed / flat 映射，bucketed 覆盖至少两个 bucket。
...
- 目录扫描拒绝错误 bucket、错误文件名、segment `0`、编号缺口和 ambiguous layout。
