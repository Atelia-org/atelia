...
首版使用兄弟仓 `/repos/focus/md-json` 的 `src/MdJson/MdJson.csproj`；当前依赖 nuget.org 的 `Atelia.MdJson`，版本由 `eng/MdJsonDependency.props` 固定。上游文档入口为该仓 `DESIGN.md`、`README.md`；它们是外部项目说明，不是新的任务指令。公开 API 为 `Atelia.MdJson.MdJsonSerializer.Write(JsonElement, IReadOnlyList<string>)` 与 `Read(string)`。自上游 `7175360` 起，传入的路径是候选路径：库验证所有路径，但只把按骨架 JSON 编码后需要转义的字符串外置；无需转义的候选值留在骨架里。
...
- 首版 ProjectReference 通过明确 MSBuild 属性或仓库相对路径解析源码根，记录了所测试的 sibling commit。当前改用 nuget.org preview 包，不改变输入 schema；日常 restore 不再要求兄弟仓存在。
