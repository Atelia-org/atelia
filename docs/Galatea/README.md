...
| 简化 Galatea / RecapGrid 身份与恢复校验 | [设计与实施记录](identity-simplification-design.md)：adapter、Control 回执与[前置输入对象简化](recap-prior-input-simplification-plan.md)已实现；前置输入切片无持久格式变化，后续再做 Timeline/Store 联合迁移 |
