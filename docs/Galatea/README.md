...
| 简化 Galatea / RecapGrid 身份与恢复校验 | [设计与实施记录](identity-simplification-design.md)：前三切片已实现；[Store 简化](recap-store-simplification-plan.md)也已实现并通过 1,771 项本地测试，尚未部署。旧 Recap 不转换，全部重构完成后最后统一重建 |
