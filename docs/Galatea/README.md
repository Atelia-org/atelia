...
| 简化 Galatea / RecapGrid 身份与恢复校验 | [设计与实施记录](identity-simplification-design.md)：前三切片及 [Store 简化](recap-store-simplification-plan.md)已实现；下一入口为 [Timeline 单一行身份计划](timeline-row-identity-simplification-plan.md)，已规划、待实施。旧 Recap 不转换，全部重构完成后最后统一重建 |
