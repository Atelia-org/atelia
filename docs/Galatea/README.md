...
| 简化 Galatea / RecapGrid 身份与恢复校验 | [设计与实施记录](identity-simplification-design.md)：前三切片及 [Store 简化](recap-store-simplification-plan.md)已实现；当前 [Timeline 单一行身份](timeline-row-identity-simplification-plan.md)正在实施，验证待补。旧 Recap 不转换，全部重构完成后最后统一重建 |
