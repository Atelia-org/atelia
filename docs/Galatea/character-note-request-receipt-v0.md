...
- 方案日期：2026-08-30
- 文档状态：**Historical predecessor**；已由[`Character Note Default MemoPod V1`](character-note-default-memopod-v1.md)取代；下文保留V0当时实现与证据，不描述current runtime。当前提取采用[Note 忠实代写](character-note-transcription.md)，下文`ExactText`/`EvidenceQuote`逐字子串规则与提交句式限制已失效。
