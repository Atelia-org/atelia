...
设计并原型实现一个 StateJournal 层的离线 commit address reader。初版基于 branch reflog / branch ref v2 `recentHeads`，用于离线分析和救援场景；raw reader 枚举的是“曾出现在 branch metadata 中的 `CommitAddress` 候选集”，不是完整 commit DAG，也不默认反序列化对象图。

本任务的语义需要收紧为：reflog-based branch history address enumeration。`oldHead -> newHead` 表示 branch ref transition，通常可反映 main branch 的 head 演化链，但不应被表述为通用 commit parent edge。当前实现同时提供 raw candidates 与 effective parent-chain 两层：raw candidates 保留 reflog / recentHeads 中的短旁支，effective parent-chain 从当前 HEAD 沿 commit parent metadata 反向遍历，只保留当前有效父链。未来仍可以在此基础上新增其他救援遍历方案，例如：基于 segment 扫描的 orphan commit discovery、或基于上层业务索引的恢复策略。

建议能力：

- 通过 `EnumerateBranchRawCommitAddresses(repoDir, branchName)` 枚举某 branch 的 reflog entries，从 `oldHead` / `newHead` 提取完整 `CommitAddress`。
- raw reader 枚举 branch ref v2 与 `.json.last` backup 中的 `head` / `recentHeads`，作为 reflog 缺失或损坏时的补充。
- raw reader 去重、标准化为 `CommitAddress` 序列，并保留来源信息（例如 reflog line、branch head、recent head）。
- 通过 `EnumerateBranchEffectiveCommitAddresses(repo, branchName)` 从当前 branch HEAD 沿 `HeadParentAddress` 反向遍历有效父链，排除被回退的短旁支。
