...
2. 计算 split point，构造 `prefix` / `suffix`。
3. 将 `prefix` 写入 archive 草稿。
4. 调用 secretary 产出维护草案。
5. 校验草案：非空、长度预算、必需区块存在、无明显 JSON/Markdown 破损。
6. 写入 audit 草稿。
7. 原子替换 `pack.md`。
8. 将 `prefix` 从 `ChatSession` history 移除，保留 `suffix`。
9. 提交 session 状态。

如果第 8 步前失败，不应改变聊天历史。如果第 8 步后失败，需要 audit 中有足够信息支持人工恢复。
...
建议先放在 `prototypes/Galatea` 内部，不急于泛化到 `ChatSession`：

| 类型 | 职责 |
|---|---|
| `GalateaMemoryStore` | 读取/写入 `pack.md`、audit、episode、archive |
| `GalateaMemoryMaintenanceService` | 编排 split、secretary 调用、校验、应用 |
...
| `ProjectMessagesForMaintenance(split)` | 取 prefix/suffix 的只读投影 |
| `ReplacePrefixWithRecap(...)` 或 `RemovePrefix(...)` | 让上层维护器应用自己的压缩结果 |
| `ContextHeader` | 在 history 前注入结构化前段，不伪装成 user recap，也不整体伪装成 system prompt |

MVP 可先通过内部方法复制少量逻辑；确认方向后再回收成公共 API。
...
2. 实现 `GalateaMemoryStore` 的文件版：`pack.md`、`audit.jsonl`、`episodes.jsonl`、`archives/`。
3. 在系统提示词中加入 Hint 规则，并实现 `<hint>` 提取的纯文本测试。
4. 做单次 secretary 维护调用，产出 `GalateaMemoryMaintenanceDraft` JSON。
5. 在 post-generation compaction 位置接入维护服务，但先只写 pack/audit/archive/context header，不删除 history。
6. 确认 pack 质量后，再让维护服务替代现有 `CompactAsync(...)` 的历史删除/替换行为。
7. 增加重吸收 reviewer 的构造测试。
8. 把文件版 memory store 迁入 StateJournal，或把 `ContextHeader` 提取到更公共的 completion/session 抽象层。
