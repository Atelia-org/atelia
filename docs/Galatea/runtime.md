...
MemoPod recall 的 `invalid-model-output` 在主模型 `SendAsync` 前最多尝试两次；provider/transport 重试仍由 Completion decorator 负责。无效输出不会被当作空召回。`Galatea.MemoRecall` 的 `memo-recall-planning` schemaVersion 2 在最终失败时记录 `outputFailureCode`、`selectorAttempts` 与 `turnId`，首次失败而第二次成功时另有 `Selector output rejected/recovered` Debug 记录。日志只记录受限代码、次数和角色/轮次标识，不记录 provider 原文。若自主回合失败后显示 `autonomy-paused/AUTONOMOUS_TURN_FAILED`，页面的“恢复自主活动”调用[显式恢复入口](server-api.md)，在 Idle 边界重新计时；它不会重放失败回合。
