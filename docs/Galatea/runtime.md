...
新 Prepared 写 v9 语义计划，Observation/Setup 写 v2，Started v2 保存本次请求承诺；旧 v7/v8 保持 exact 恢复，v5 保持历史审计可读、completion 不可执行。部署前正常停服并保留完整数据快照，
...
`GalateaTurnException` 与输入超限阻止新轮次。取得 effective text 后才重验可放弃的旧失败 turn。
...
Delivered 只证明 Observation append，不证明 provider 收到或理解，abandon/rewind 前先结算。
...
2. **`Started`**：先纯验证持久事实；默认 `Refuse` 不要求 projector，且发生在本次 current connection selection/client、route 和 derived owner 之前。显式授权新 attempt 才可重发，换 renderer 本身不产生授权。
