...
> **归档说明（2026-07-22）**：该交接任务已经完成，下面的提示词描述的是施工前状态，不应再次执行。现行 substrate 与 Rewrite-only 决策见 `prototypes/ChatSession/MemorySubstrate.cs` 和 `docs/Galatea/backlog/done/memory-maintainer-slimming-refactor.md`。
