...
operator-owned character context
        + "\n\n---\n\n"
code-owned mailbox protocol suffix
...
来源边界。Operator不能从root config覆盖或删除这段协议。
...
覆盖inline并在strict UTF-8 decode后`Trim()`。它与prefix/suffix一起只render一次，因此replacement不会递归成为
template语法。

### 3.3 Mailbox protocol suffix

[`prompt/trpg-mailbox-protocol-suffix-zh-cn.md`](prompt/trpg-mailbox-protocol-suffix-zh-cn.md)由Galatea.Server
embedded resource拥有。它定义角色如何理解、发送和接收界外邮件，与mail extractor对“实际完成发送”的判定形成
同一产品协议。Current composer无条件加入该suffix；它不是per-user可选module。

## 4. Fixed composition，而不是文档解析

`GalateaSystemPromptComposer`只做一个固定三段组合。Prefix、context、suffix之间各插入exact
`"\n\n---\n\n"`，然后调用已有`${characterName}` / `${playerName}` renderer一次。External context与每份
embedded resource有1 MiB读取cap，composite source与final rendered prompt也分别受1 MiB cap；embedded resources
还要求BOM-less、LF-only、nonblank strict UTF-8。

Markdown heading只帮助LLM与人阅读，不是machine discriminator。Runtime不会扫描`## 世界观`、`## 自主记忆`或
`## 界外邮箱机制`来决定分段、权限、feature或安全性。Operator可以在context中写任意合法Markdown prose，但无法
借同名H2覆盖code-owned协议。
...
- **不做module/include engine。** 当前只有固定三段，不需要module ID、priority、optional list、include cycle、
  per-module digest或动态registry。
...
- Prepared/Frozen recovery继续读取historical governing setup与frozen request，不用current三段重组。
- RecapGrid V6仍由character/player names决定Definition/BuildTarget；主prompt拆分不旋转asset。
- Outbound extractor继续使用自身code-owned prompt和ContractId；主prompt suffix不进入extractor identity。
...
旧`cyber.md`已经包含与current suffix等价的界外邮箱段；正确切分可以保持这部分语义。旧`gpt.md`没有邮箱段，
V5 composer会为它新增同一code-owned mailbox appendix；这是统一runtime protocol的预期变化，不是需要per-user
module开关的兼容问题。未被config引用的`cyber_template.md`不阻塞迁移。
...
| Composition | exact prefix/separator/context/separator/suffix；只render一次；无第四份full prompt |
| Bounds/resources | external context、composite source与final output各1 MiB；embedded resource strict UTF-8、BOM-less、LF-only |
| Runtime DTO | 每个user只保留validated names与finalized `SystemPrompt` |
| Bootstrap | missing in-root context create-only、shared path一次、existing no-overwrite、protocol不落operator目录 |
| Durable setup | new repository保存final prompt；Idle只按exact变化rotate；Frozen使用historical setup |
