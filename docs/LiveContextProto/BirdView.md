...
- **明确 RecapMaintainer 的触发时机与资源**：可能在 `WaitingInput` 中检测历史长度超阈值，或在 `ProcessToolResultsReady` 完成后由更高层宿主异步触发。需要评估其使用的模型/工具预算，与 Memory Notebook 协作策略（Recap 是否写入 Notebook 或独立文件）。
