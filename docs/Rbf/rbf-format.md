...
### spec [F-FILE-MINIMUM-LENGTH] 最小文件长度
> 有效 RBF 文件长度 MUST >= 4（至少包含 Genesis Fence）。
> `fileLength < 4` 表示文件不完整或损坏，Reader MUST fail-soft（返回空序列），MUST NOT 抛出异常。

### derived [H-FILE-MINIMUM-LENGTH-RATIONALE] 最小文件长度设计理由
> fail-soft 策略与接口层 @[S-RBF-SCANREVERSE-EMPTY-IS-OK] 保持一致，使上层可以统一处理"无有效帧"的情况，而无需区分"文件不完整"与"文件为空但合法"。
---
...
### spec [F-FRAME-LAYOUT] FrameBytes布局
...
> `FrameTag` 的接口层定义见 [rbf-interface.md](rbf-interface.md) §2。

### spec [F-FRAMESTATUS-VALUES] FrameStatus位域定义
...
### derived [H-FRAMESTATUS-DESIGN-RATIONALE] FrameStatus设计理由
...
### spec [F-HEADLEN-FORMULA] HeadLen计算公式
> HeadLen = 4 (HeadLen) + 4 (FrameTag) + PayloadLen + StatusLen + 4 (TailLen) + 4 (CRC32C)

### spec [F-STATUSLEN-FORMULA] StatusLen计算公式
...
>
> 合法值由 @[F-FRAMESTATUS-VALUES] 位域 SSOT 定义。
...
### spec [R-REVERSE-SCAN-ALGORITHM] 逆向扫描契约
...
### spec [R-RESYNC-BEHAVIOR] Resync行为规则
...
### spec [R-DATATAIL-DEFINITION] DataTail定义
