...
`ReverseReadCache` 仍不做复杂的 stride detection，也不额外维护正向/逆向两套缓存。当前采用的低成本折中是：当一次小读跨越两个页时，已经发生了一次 raw read，因此直接读取并缓存完整的 2-page window。这样逆序 scan 后续通常命中起始页，正向 scan 后续通常命中结束页；代价只出现在跨页 miss 上，不改变普通单页小读路径。
