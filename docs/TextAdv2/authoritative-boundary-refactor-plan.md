...
- 让 `TextAdv2Runtime` 的核心行为以更明确的 typed methods 暴露出来。
- 让 `Execute(TextAdv2RuntimeCommand)` 退化为薄适配层。
- 提炼 sample-world bootstrap 入口，让 dev/bootstrap 语义更明确。
- 让 host 更直接依赖 typed seam，而不是继续扩张命令枚举耦合。

完成标准：

- `TextAdv2Runtime` 至少具备覆盖现有主行为的 typed methods。
