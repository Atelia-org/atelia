...
结合当前代码、测试与宿主形态，F7 已经收完后，真正还值得留在主计划里的问题已经缩到下面两类。

### 2.1 两个宿主的 machine JSON contract 仍然分叉

当前 `GameServer` 与 `E2eCli` 都已经能提供可机读 seam，但它们还没有形成同一份 canonical machine contract。

当前实际表现：

- `GameServer` JSON 走 camelCase
- `E2eCli` JSON 仍走 PascalCase
- `TextAdv2.Tests` 当前也分别锁定了这两套 casing
- 真正的 Agent / PipeMux / 脚本消费方如果想同时支持两个宿主，仍然要按宿主分支处理字段名

这已经不是“风格差异”，而是明确的 cross-host contract 分叉。

如果 TextAdv2 近期目标真的是“可给 Agent 跑的空间沙盒 / RL Agent Gym”，那么这个分叉比继续补零散功能更值得优先收口。
...
1. `F8a`：canonical JSON contract 统一
2. `F8b`：cross-host parity 测试与文档收口
3. `F9`：runtime boundary freeze
4. `T1`：只在真实创作压力出现时补 host authoring parity
5. `T3`：durable action log / replay backlog
6. `T2`：只在真实生命周期压力出现时补 bootstrap parity
7. `T4`：richer gameplay backlog

这个顺序比旧计划更合理，因为：

- `F7` 已经完成，继续围绕它排计划只会制造文档噪音
- 它直接对准当前最真实的跨宿主 contract 分叉
