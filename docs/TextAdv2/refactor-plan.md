...
- legacy sidecar-only 目录不会再被静默重建成 sample world，而会按“非有效 repo”进入显式失败路径
- sample-world-dev host 在这种 legacy sidecar-only 目录上会进入 `open-failed` / `degraded`
- `POST /admin/reset-sample-world` 仍可用来删除整个 dev repo 目录并重建 sample world，但这是显式 reset 动作，不是对任意杂质目录都安全的自动兼容逻辑
