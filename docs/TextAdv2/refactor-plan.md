...
> 状态：按 2026-06-01 当前代码与测试基线修订（含 F9 第一拍、F10 第一拍、T1 第一拍）
...
`F10` 已经把 host failure contract 收住，`T1` 也已经完成了一拍 read-only observation/navigation slice。
...
- `plan-route`、`create-actor`、`create-passage`
...
- 当前已完成一拍 read-only observation/navigation slice：
  - `observe-location`
  - `observe-navigation`
  - `observe-actor-navigation`
- 优先：
