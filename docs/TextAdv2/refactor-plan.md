...
- `GameServer` 已引入显式 host/session 双层状态：
  - `host.readiness = "alive"`
  - `session.readiness = "ready" | "open-failed"`
- `/admin/session-status` 与 `/` 当前始终返回 `200`，并暴露 machine-consumable 的 session readiness
- `/healthz` 当前在 session ready 时返回 `200 + status=ok`，在 open-failed 时返回 `503 + status=degraded`
- open-existing-only + repo 缺失时，status/health 不再伪装成 ready
- `reset-sample-world` 失败不再静默返回假阳性 `200 OK`

当前刻意没在这一拍继续扩的部分：

- 不统一其他 session-backed endpoint 的 failure JSON contract
- 不处理 CLI 纯机读模式
- 不处理 route trace JSON seam
- 不把 retry/recovery 策略扩成更大的 host lifecycle 包
...
- `E2eCli` 已引入显式 `--json-only` 机读模式
- `--json-only` 当前只接受一条 JSON 类 operation，并在 session 打开前 fail-fast 校验
- 纯机读模式不再混入 `TextAdv2 session repo:`、`[1/2]` 之类的人类调试输出
- 文本 operation 语义保持原样，没有被 `--json-only` 偷偷改写
- 黑盒测试当前已覆盖：
  - 单 JSON operation 输出纯 JSON
  - 多 JSON operation fail-fast
  - 文本 operation fail-fast
  - 无显式 operation fail-fast
  - help / usage 暴露 `--json-only`

当前刻意没在这一拍继续扩的部分：

- 不把所有 session 输出都强行改成默认机读
- 不追求“stdout 绝对零 side-channel”之外的更大 host 约束
- 不扩成更复杂的多命令 batch machine protocol
...
- `GameServer` 保留了原有文本 `GET /actors/{actorId}/route-trace`
- `GameServer` 另外增加了并行 JSON seam：`GET /actors/{actorId}/route-trace/json`
- `E2eCli` 保留了 `--trace-actor-route <actorId>` 文本调试入口
- `E2eCli` 另外增加了 `--trace-actor-route-json <actorId>`，并自然支持 `--json-only`
- 两个宿主都直接复用现有 `ActorRouteTrace`，没有再新造 route-trace DTO
- reviewer 指出的三处当前包内尾项也已补齐：
  - `plannedEndpoints` 已补回 `/`、`/healthz`、`/admin/session-status`
  - `E2eCli help` 已显式列出文本 / JSON 两条 route trace surface
  - 测试已补锁文本 trace 在 reset / reopen 后仍回到 `steps=0`

当前刻意没在这一拍继续扩的部分：

- 不把 route trace renderer 改造成更复杂的统一 descriptor 体系
- 不继续扩展更多 machine-only gameplay seam
- 不把 `plannedEndpoints` 再抽成更大的自动注册基础设施
...
- `F7a` 已经把 GameServer 的 readiness/failure seam 收口，所以当前下一步自然转向 CLI 的 machine-output 问题
