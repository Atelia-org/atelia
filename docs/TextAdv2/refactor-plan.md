...
结合当前代码、测试与宿主形态，F8a 落下后，真正还值得留在主计划里的问题已经缩到下面两类。

### 2.1 cross-host parity 还没有被正式锁成长期契约

当前 `GameServer` 与 `E2eCli` 的 JSON casing 已经对齐到 camelCase，但这还只是“canonical contract 第一拍成立”，还没有变成一组清晰的长期 parity guard。

当前实际表现：

- `GameServer` 与 `E2eCli` 分别有各自的测试
- 但还缺少一批明确命名的 cross-host parity 断言
- 文档里也还没有把“哪几条 machine surface 才是 canonical contract”写成稳定承诺

这意味着：

- 当前代码已经更接近统一 contract
- 但后续再加 machine seam 时，仍然有机会悄悄再次长出 host-specific 漂移

所以接下来更合理的主问题不再是“继续修 casing”，而是完成 `F8b`，把 parity guard 和文档承诺一起补齐。

### 2.2 runtime-owned state 的长期边界已经浮现，但还没有被正式冻结

当前代码与测试其实已经形成了非常清楚的三分法：
...
问题不在于“实现不工作”，而在于这些边界现在还主要散落在注释、测试和局部命名里，没有被提升成一份明确的项目级决定。

这会带来两个后续风险：

- 后面很容易有人把“trace / cache 是不是也要 durable”重新当成进行中的开放问题
- 现存的旧兼容尾巴，例如 sample bootstrap 中对 `.textadv2-runtime-state.json` 的 legacy sidecar 清理逻辑，会继续模糊真实边界

结合当前目标，更合理的方向不是“把现有 trace / cache 直接 durable 化”，而是先正式冻结边界：

- 现有 `movement trace` 继续定义为 runtime debug seam
- 现有 `route acceleration` 继续定义为 runtime cache
- 如果未来真要 durable replay / audit / curriculum scoring，再新开一个 world-backed action log 包，而不是复用当前 trace/cache 类型
...
1. `F8b`：cross-host parity 测试与文档收口
2. `F9`：runtime boundary freeze
3. `T1`：只在真实创作压力出现时补 host authoring parity
4. `T3`：durable action log / replay backlog
5. `T2`：只在真实生命周期压力出现时补 bootstrap parity
6. `T4`：richer gameplay backlog

这个顺序比旧计划更合理，因为：

- `F7` 与 `F8a` 都已经完成，继续把它们写成“下一步”只会制造文档噪音
- 它直接对准当前最真实的剩余 contract 风险，也就是 parity guard 还没有完全钉住
- 它把“runtime state 要不要 durable”从模糊争论改写成更清晰的边界冻结
- 它把将来真正可能需要的 durable replay 能力单独放回正确问题域
- 它避免在 machine contract 还未统一时过早讨论更重玩法
...
- actor context / move result / route trace / time 在两个宿主上的 machine contract 一致
