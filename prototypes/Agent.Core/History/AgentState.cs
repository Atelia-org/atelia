...
memory_notebook_replace与memory_notebook_replace_span工具就是为你主动编辑你自己的[Memory Notebook]而设计的。";
    ...
    public void AppendNotification(string item) {
        ...
        SyncSessionAppendedNotification(item);
    ...
    internal string? TakeoutPendingNotifications() {
        EnsureWorkspaceSessionOpen();
        var drained = _workingSet.DrainPendingNotifications();
        if (drained.Length == 0) { return null; }

        SyncSessionPendingNotifications();
    ...
    private void AppendEntryCore(HistoryEntry entry) {
        ...
        SyncSessionAppendedHistoryEntry(entry);
    ...
    internal RecapEntry ReplacePrefixWithRecap(int splitIndex, string summary) {
        ...
        SyncSessionHistoryAndSerial();

        DebugUtil.Info(
            "History",
            $"Compacted prefix replaced={splitIndex} insteadSerial={insteadSerial} recapSerial={recap.Serial} remaining={RecentHistory.Count}"
        );
