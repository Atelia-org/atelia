...
/// 仅保留最近的若干条目用于实时上下文渲染与反射操作。更早的历史条目会通过
/// LLM 一次性摘要生成 RecapEntry 后沉入持久存储，不再占用内存。
/// 与持久历史的分工：
...
memory_notebook_replace与memory_notebook_replace_span工具就是为你主动编辑你自己的[Memory Notebook]而设计的。";
    ...
    public ObservationEntry AppendObservation(ObservationEntry entry, LevelOfDetailContent? inlineNotifications = null) {
        if (entry is null) { throw new ArgumentNullException(nameof(entry)); }
        ValidateAppendOrder(entry);
    ...
    private void ValidateAppendOrder(HistoryEntry entry) {
        ...
        var lastIsObservation = last.IsObservationLike;
        var nextIsObservation = entry.IsObservationLike;

        // 这里强制 Observation ↔ Action 的交替顺序，一方面让 AgentEngine 的状态机有稳定前提，
        // 另一方面也为摘要等只读视图提供"交错配对"的结构保障。
    ...
    /// <c>_recentHistory[splitIndex]</c> 为 <see cref="ActionEntry"/>。
    /// 替换后序列仍满足 Observation↔Action 交替不变式。
    /// <para>
    ...
    internal RecapEntry ReplacePrefixWithRecap(int splitIndex, string summary) {
        ...
        Debug.Assert(_recentHistory[splitIndex] is ActionEntry, "Suffix must start with ActionEntry.");
        ...
        recap.AssignSerial(++_lastSerial);
    ...
    private int DetermineActiveTurnStartIndex(CurrentTurnInfo currentTurn) {
        ...
        for (var index = 0; index < _recentHistory.Count; index++) {
            if (_recentHistory[index] is ActionEntry) { return index; }
        }
    ...
    /// </summary>
    private void AttachNotificationsToObservation(ObservationEntry entry, LevelOfDetailContent? inlineNotifications = null) {
    ...
    private static LevelOfDetailContent? MergeNotifications(LevelOfDetailContent? first, LevelOfDetailContent? second) {
        if (first is null) { return second; }
        if (second is null) { return first; }
        return LevelOfDetailContent.Join("\n", first, second);
    }
}
