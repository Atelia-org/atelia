...
memory_notebook_replace与memory_notebook_replace_span工具就是为你主动编辑你自己的[Memory Notebook]而设计的。";
    ...
    public ProjectedInvocationContext ProjectInvocationContext(ContextProjectionOptions options) {
        ...
        for (var index = _recentHistory.Count; --index >= 0;) {
            ...
            var projected = ProjectHistoryEntry(_recentHistory[index], options, currentTurn, isInActiveTurn, ref pendingWindows);
            if (isInActiveTurn) {
    ...
    private static IHistoryMessage ProjectHistoryEntry(
        ...
        return entry switch {
            ...
            ObservationEntry observationEntry => ProjectObservationEntry(observationEntry, ref pendingWindows),
    ...
    private static ObservationMessage ProjectObservationEntry(
        ...
        var projected = observationEntry.GetMessage(pendingWindows);
        pendingWindows = null;
        return projected;
    }
