...
/// </summary>
/// <remarks>
/// <para><b>切分点合法性约束</b>：切分只能发生在 observation-like → actor-side 边界上。</para>
/// <para>
/// 初始 Observation 可以作为 prefix 的最后一条被摘要，也可以作为 suffix 的第一条被保留——
/// 当前实现选择前者。切分后 prefix 以 Observation-like 结束，suffix 以 actor-side entry 开始，
/// 交替结构保持合法。
/// </para>
/// <para><b>返回值语义</b>：返回 suffix 的起始索引（即切分后第一条 actor-side entry 的位置）。
/// 调用方可使用 C# 右开区间进行切片：
...
public static class ContextSplitter {
    ...
    /// <item>第二遍：再次遍历，在每个 observation-like → actor-side 边界检查累计 token 数是否达到 ceiling(总量/2)，
    ...
    public static int FindHalfContextSplitPoint(IReadOnlyList<HistoryEntry> entries) {
        // 至少需要 2 条才能形成 observation-like → actor-side 边界
        ...
        for (int i = 0; i < entries.Count - 1; i++) {
            ...
            if (entries[i].IsObservationLike && entries[i + 1].IsActorLike) {
                int suffixStart = i + 1;
