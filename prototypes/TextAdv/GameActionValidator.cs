...
internal static class GameActionValidator
{
    ...
    private static readonly ImmutableArray<ToolDefinition> s_tools = [
        new ToolDefinition(
            ...
            Description: "指出不合理之处。仅当你发现这一步动作或其 Reason-Trace 不 grounded、有跳步、偷用输入外记忆、或把猜测写成确定事实时调用。若动作合理，请不要调用任何工具。",
            Parameters: [
                new ToolParamSpec(
    ...
    internal static async Task<ValidationResult> ValidateActionAsync(
        GameSimulation.PerceptionBundle perception,
        string actionKind,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var request = new CompletionRequest(
            ...
            {
                new ObservationMessage(BuildObservation(perception, actionKind, actionSummary, reasonTrace, actionPayload))
            },
            Tools: s_tools);
...
你的任务不是判断玩家是否猜中世界真相，而是判断这一步动作的 Reason-Trace 是否 grounded 于给定证据。
...
4. 标准是“防君子不防小人”的 groundedness 检查，不追求吹毛求疵。
...
""";
        ...
        string? actionPayload)
    {
        ...
        sb.AppendLine($"- Day: {perception.Day}");
        sb.AppendLine($"- Slot: {perception.Slot}/{perception.SlotsPerDay}");
        ...
        sb.AppendLine();
        sb.AppendLine("[Memory-Notebook 当前快照]");
        sb.AppendLine(string.IsNullOrWhiteSpace(perception.NotebookContent) ? "(empty)" : perception.NotebookContent);
        ...
        else
        {
            foreach (var step in perception.AcceptedSteps)
            {
                ...
                sb.AppendLine($"   reason: {step.ReasonTrace}");
            }
        }
        ...
        sb.AppendLine();
        sb.AppendLine("[Reason-Trace]");
        sb.AppendLine(reasonTrace);
    ...
    private static string BuildRejectedFeedback(IReadOnlyList<RawToolCall> toolCalls)
    {
        ...
        if (issues.Length == 0)
        {
            return "未通过：模型调用了问题指出工具，但未给出可读内容。";
        }
    ...
    private static string RequireEnvironment(string key)
    {
        ...
        {
            throw new InvalidOperationException($"Environment variable '{key}' is required for DeepSeek validator.");
        }
