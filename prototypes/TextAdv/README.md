...
- LLM Player Agent 首版开放 `player_edit_memory_notebook` Small-Action，以及 `player_rest_a_while`、`player_explore`、`player_interact` 三个 Large-Action 工具；运行时要求真实 provider 可用，失败时直接报错。若诊断跑批不想启用真实内部玩家，可在更外层改用保守诊断动作
...
- `gm_move_player` 是 `gm_move_actor(player, ...)` 的便捷别名
- `gm_create_item`
- `gm_create_npc`
