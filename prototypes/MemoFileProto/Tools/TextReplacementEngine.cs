...
internal sealed class TextReplacementEngine {
    ...
    public TextReplacementEngine(Func<string> getMemory, Action<string> setMemory) {
    ...
    public ReplacementOutcome Execute(ReplacementRequest request, IRegionLocator? locator) {
        try {
            ...
            return ReplacementOutcome.Succeed(
                ToolMessages.FormatUpdate(locateResult.StartIndex, locateResult.Length, updatedMemory.Length)
            );
        }
        catch (Exception ex) {
    ...
    private ReplacementOutcome Append(string currentMemory, string newText) {
        ...
        if (!hasNewContent) { return ReplacementOutcome.Succeed(ToolMessages.FormatNoContentToAppend(currentMemory.Length)); }
        ...
        return ReplacementOutcome.Succeed(ToolMessages.FormatAppendSuccess(appendedMemory.Length));
    }
