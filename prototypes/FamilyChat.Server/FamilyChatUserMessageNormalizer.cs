...
public interface IFamilyChatUserMessageNormalizer {
    bool ShouldNormalize(string userMessage);

    ValueTask<string> NormalizeAsync(string userMessage, CancellationToken ct);
}

public sealed class DisabledFamilyChatUserMessageNormalizer : IFamilyChatUserMessageNormalizer {
    public static DisabledFamilyChatUserMessageNormalizer Instance { get; } = new();

    private DisabledFamilyChatUserMessageNormalizer() { }
...
internal static class FamilyChatUserMessageNormalizerFactory {
    private const string BaseUrlEnvVar = "DEEPSEEK_BASE_URL";
    private const string ApiKeyEnvVar = "DEEPSEEK_API_KEY";
    private const string DebugCategory = "FamilyChat.Input";

    public static IFamilyChatUserMessageNormalizer CreateFromEnvironment() {
        ...
        if (string.IsNullOrWhiteSpace(baseUrl) && string.IsNullOrWhiteSpace(apiKey)) {
            ...
            return DisabledFamilyChatUserMessageNormalizer.Instance;
        }
        ...
        return new DeepSeekFamilyChatUserMessageNormalizer(parsedBaseUrl, apiKey);
    }
}

public sealed class DeepSeekFamilyChatUserMessageNormalizer : IFamilyChatUserMessageNormalizer, IDisposable {
    private const string DebugCategory = "FamilyChat.Input";
...
""";
    ...
    public DeepSeekFamilyChatUserMessageNormalizer(Uri baseAddress, string apiKey) {
    ...
    public async ValueTask<string> NormalizeAsync(string userMessage, CancellationToken ct) {
        ...
        if (!ShouldNormalize(userMessage)) {
            return userMessage;
        }
