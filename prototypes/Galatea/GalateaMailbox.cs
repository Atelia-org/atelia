...
internal sealed record InMemorySendMailIntentCandidate(
    string UserId,
    string TurnId,
...
internal sealed class InMemorySendMailIntentBuffer {
    internal const int MaximumCandidateCount = 256;
    internal const int MaximumCandidateUtf8Bytes = 2 * 1024 * 1024;

    private sealed record Batch(
        EventAddress SourceActionHead,
        IReadOnlyList<InMemorySendMailIntentCandidate> Candidates,
        int Utf8Bytes
    );
    ...
    internal bool TryAddBatch(
        ...
        InMemorySendMailIntentCandidate[] candidates = intents
            .Select((intent, ordinal) => new InMemorySendMailIntentCandidate(
                userId,
                turnId,
                sourceActionHead,
                ordinal,
        ...
        int bytes = candidates.Sum(EstimateUtf8Bytes);
        if (candidates.Length > MaximumCandidateCount
            || bytes > MaximumCandidateUtf8Bytes) {
            throw new InvalidOperationException(
                "A send-mail intent batch exceeds the in-memory buffer bounds."
            );
        }

        lock (_gate) {
            if (_seenActionHeads.Contains(sourceActionHead)) { return false; }
            if (_candidateCount + candidates.Length
                    > MaximumCandidateCount
                || _utf8Bytes + bytes
                    > MaximumCandidateUtf8Bytes) {
                throw new InvalidOperationException(
                    "The in-memory send-mail intent buffer is full."
                );
            }
            _seenActionHeads.Add(sourceActionHead);
            _batches.AddLast(new Batch(
                sourceActionHead,
    ...
    internal void Remove(EventAddress sourceActionHead) {
        lock (_gate) {
            LinkedListNode<Batch>? node = _batches.First;
            while (node is not null) {
                LinkedListNode<Batch>? next = node.Next;
                if (node.Value.SourceActionHead == sourceActionHead) {
                    _candidateCount -= node.Value.Candidates.Count;
                    _utf8Bytes -= node.Value.Utf8Bytes;
                    _batches.Remove(node);
                    break;
                }
                node = next;
            }
            _seenActionHeads.Remove(sourceActionHead);
        }
    }

    internal IReadOnlyList<InMemorySendMailIntentCandidate> Snapshot() {
        lock (_gate) {
            return Array.AsReadOnly(_batches
                .SelectMany(static batch => batch.Candidates)
                .ToArray());
        }
    }

    private static int EstimateUtf8Bytes(
        InMemorySendMailIntentCandidate candidate
    ) => Encoding.UTF8.GetByteCount(candidate.UserId)
        + Encoding.UTF8.GetByteCount(candidate.TurnId)
        + Encoding.UTF8.GetByteCount(candidate.Intent.Recipient)
        + Encoding.UTF8.GetByteCount(candidate.Intent.Subject ?? string.Empty)
        + Encoding.UTF8.GetByteCount(candidate.Intent.Body)
        + Encoding.UTF8.GetByteCount(
            candidate.Intent.InReplyToMessageId ?? string.Empty
        )
        + Encoding.UTF8.GetByteCount(candidate.Intent.EvidenceQuote)
        + 64;
}
