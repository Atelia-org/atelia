    ...
    private static SessionRequest ParseSessionRequest(string[] args) {
        ...
        while (index < args.Length) {
            switch (args[index]) {
                ...
                break;
                case "--trace-actor-runtime-route": {
                    string actorId = RequireArg(args, index + 1);
                    operations.Add(
                        new SessionCommand(
                            $"trace actor runtime route {actorId}",
                            SessionCommandOutputKind.Text,
                            session => DevTextRenderer.RenderRuntimeRouteTrace(session.TraceActorRuntimeRoute(actorId))
                        )
                    );
                }
                index += 2;
                break;
                case "--trace-actor-runtime-route-json": {
                    string actorId = RequireArg(args, index + 1);
                    operations.Add(
                        new SessionCommand(
                            $"trace actor runtime route json {actorId}",
                            SessionCommandOutputKind.Json,
                            session => RenderJson(session.TraceActorRuntimeRoute(actorId))
                        )
                    );
                }
                index += 2;
...
Session options:
    ...
    --trace-actor-runtime-route <actorId>
                     Print the text runtime route trace for human debugging.
    --trace-actor-runtime-route-json <actorId>
                     Print the typed actor runtime route trace as JSON.
"""
        );
