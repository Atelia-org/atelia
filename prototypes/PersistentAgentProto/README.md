...
`PersistentSession.BuildContext()` 把上述 durable graph 投影成 `IReadOnlyList<IHistoryMessage>` 喂给 `ICompletionClient`。`assistant` 一侧用一个本地的 `TextOnlyAction` record 当 `IRichActionMessage`（无 tool_call、无 thinking）——这是下一刀的扩展点。
...
要把当前的"纯文本 message"扩展到完整 `IRichActionMessage`（OpenAI/Anthropic converter 的全集），需要把以下结构持久化进 message dict：
