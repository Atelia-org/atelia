...
public sealed class SymbolIndex : ISymbolIndex
{
    ...
    private void AddType(INamedTypeSymbol t)
    {
        ...
        _all.Add(entry);
        if (!_fqnCaseSensitive.ContainsKey(fqn)) _fqnCaseSensitive[fqn] = docId;
        if (!_fqnIgnoreCase.ContainsKey(fqn)) _fqnIgnoreCase[fqn] = docId;
    ...
    public async Task<IReadOnlyList<SearchHit>> SearchAsync(string query, CodeCortexV2.Abstractions.SymbolKind? kindFilter, int limit, CancellationToken ct)
    {
        if (string.IsNullOrWhiteSpace(query)) return Array.Empty<SearchHit>();
        query = query.Trim();
        var added = new HashSet<string>(StringComparer.Ordinal);
        var results = new List<SearchHit>(Math.Max(8, limit));
        ...
        if (TryResolveSymbolId(query, out var idSym))
        {
            if (idSym is INamedTypeSymbol nts)
            {
                ...
                AddResult(results, added, new SearchHit(disp, ToKind(nts), nts.ContainingNamespace?.ToDisplayString(), nts.ContainingAssembly?.Name, new SymbolId(docId), MatchKind.Id, Score: 0), limit);
                return results;
            }
        }
        ...
        {
            var e = FindEntry(id1);
            if (e != null) AddResult(results, added, e.ToHit(MatchKind.Exact, 0), limit);
        }
        // 2) Exact FQN (ignore case)
        if (results.Count < limit && _fqnIgnoreCase.TryGetValue(query, out var id2))
        {
            var e = FindEntry(id2);
            if (e != null) AddResult(results, added, e.ToHit(MatchKind.ExactIgnoreCase, 10), limit);
        }
        // 3) Suffix
        ...
        // Ambiguity marking for suffix simple-name queries can be added later (extend SearchHit if needed)
        return results;
    }

    public async Task<SymbolId?> ResolveAsync(string identifierOrName, CancellationToken ct)
    {
        var list = await SearchAsync(identifierOrName, null, limit: 2, ct).ConfigureAwait(false);
        if (list.Count == 1) return list[0].SymbolId;
        return null;
    }
