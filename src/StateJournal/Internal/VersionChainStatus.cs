...
internal struct VersionChainStatus {
    ...
    const uint PerFrameOverhead = 6; // 涉及多个变长字段，还挺不好估算的，约34B。RbfFrame=24B, PreviousVersion+CumulativeCost=VarInt(8B+4B), RemoveCount+UpsertCount=VarInt(4B+4B)。

    // 前一个version的RBF Frame Ticket。_previousVersion不用于标记是否为rebase version, 对于 rebase version 依然可能尽可能保持不断版本历史，
    SizedPtr _committedVersion;

    // 从base（含）到_previousVersion（含）总共的开销。用于评估是否值得以deltify方式保存而非rebase。
    uint _cumulativeCost; // 由于PerFrameOverhead的存在，在初始化后_cumulativeCost不可能为0，可用于初始化检测。

    internal readonly SizedPtr CommittedVersion => _committedVersion;
    internal readonly bool HasCommittedVersion => _cumulativeCost != 0;
    /// <summary>Load 完成后修正 _previousVersion，使其指向已加载版本本身而非其前驱帧。</summary>
    internal void SetLoadedVersionTicket(SizedPtr ticket) => _committedVersion = ticket;
    ...
    internal readonly void WriteDeltify(IDiffWriter writer, uint deltifySize) {
        ...
        writer.BareUInt64(_committedVersion.Serialize(), false);
    ...
    internal void UpdateDeltified(SizedPtr versionTicket, uint deltifySize) {
        uint newCumulativeCost = GetDeltifiedCumulativeCost(deltifySize);
        _committedVersion = versionTicket;
