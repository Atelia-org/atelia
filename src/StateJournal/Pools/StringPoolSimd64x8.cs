...
internal sealed class StringPoolSimd64x8 : IMarkSweepPool<string> {
    private const int MaxPassthroughLength = 40;
    private const int IdentityCacheWays = 8;
    ...
    public SlotHandle Store(string value) {
        ...
        if (TryGetIdentityCacheSimd(value, out SlotHandle handle)) {
            return handle;
        }
    ...
    private bool TryGetIdentityCacheSimd(string value, out SlotHandle handle) {
        if (!Avx2.IsSupported) {
            return TryGetIdentityCacheScalarHashed(value, out handle);
        }

        int identityHash = RuntimeHelpers.GetHashCode(value);
        int baseIndex = GetSetBaseIndexFromIdentityHash(identityHash);
        ref int hashesRef = ref _identityHashes[baseIndex];
        Vector256<int> needle = Vector256.Create(identityHash);
        Vector256<int> candidates = Vector256.LoadUnsafe(ref hashesRef);
        Vector256<int> compare = Avx2.CompareEqual(candidates, needle);
        uint laneMask = compare.ExtractMostSignificantBits();

        while (laneMask != 0) {
            int lane = BitOperations.TrailingZeroCount(laneMask);
            laneMask &= laneMask - 1;

            int slotIndex = baseIndex + lane;
