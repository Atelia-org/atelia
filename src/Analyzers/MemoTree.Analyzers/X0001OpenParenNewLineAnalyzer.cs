...
public sealed class X0001OpenParenNewLineAnalyzer : DiagnosticAnalyzer {
    ...
    private static void AnalyzeInvocation(SyntaxNodeAnalysisContext ctx) {
        if (ctx.Node is InvocationExpressionSyntax inv && inv.ArgumentList is { } list) {
            AnalyzeList(ctx, list.OpenParenToken, list.CloseParenToken, list.Arguments.FirstOrDefault()?.GetFirstToken());
        }
    }
    private static void AnalyzeObjectCreation(SyntaxNodeAnalysisContext ctx) {
        if (ctx.Node is ObjectCreationExpressionSyntax oc && oc.ArgumentList is { } list) {
            AnalyzeList(ctx, list.OpenParenToken, list.CloseParenToken, list.Arguments.FirstOrDefault()?.GetFirstToken());
        }
    }
