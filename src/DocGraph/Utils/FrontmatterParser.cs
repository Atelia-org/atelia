...
public static class FrontmatterParser {
    ...
    public static bool TryParse(
        ...
        if (string.IsNullOrEmpty(content)) { return false; /* 无 frontmatter（正常情况） */ }

        // 1. 检测起始标记
        // [S-FRONTMATTER-001]：起始标记前允许空白字符，但不允许其他内容
        var trimmedStart = content.TrimStart();
        if (!trimmedStart.StartsWith(FrontmatterDelimiter)) { return false; /* 无 frontmatter */ }
        ...
        // 跳过起始标记后的换行符
        if (afterStart < content.Length && content[afterStart] == '\r') { afterStart++; }
        ...
        try {
            ...
            if (parsed == null) { return false; /* 空的 frontmatter */ }
    ...
    public static string? GetString(IReadOnlyDictionary<string, object?> frontmatter, string key) {
        if (!frontmatter.TryGetValue(key, out var value) || value == null) { return null; }
    ...
    public static List<string> GetStringArray(IReadOnlyDictionary<string, object?> frontmatter, string key) {
        ...
        if (!frontmatter.TryGetValue(key, out var value) || value == null) { return result; }
    ...
    private static bool ContainsAnchorOrAlias(string yamlContent) {
        ...
        foreach (var line in lines) {
            ...
            if (System.Text.RegularExpressions.Regex.IsMatch(line, @"&\w+")) { return true; }
