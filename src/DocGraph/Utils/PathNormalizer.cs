...
public static class PathNormalizer {
    ...
    public static string? Normalize(string? path) {
        ...
        // 4. 重新组合
        if (result.Count == 0) { return null; /* 空路径 */ }
    ...
    public static bool IsWithinWorkspace(string? path, string workspaceRoot) {
        ...
        foreach (var component in originalComponents) {
            ...
            else if (component != "." && !string.IsNullOrEmpty(component)) { depth++; }
        }
    ...
    public static string? ToWorkspaceRelative(string absolutePath, string workspaceRoot) {
        if (string.IsNullOrWhiteSpace(absolutePath) || string.IsNullOrWhiteSpace(workspaceRoot)) { return null; }
        ...
        // 提取相对路径部分
        if (normalizedAbsolute.Length == normalizedRoot.Length) { return "."; /* 就是根目录本身 */ }
    ...
    public static string? ToAbsolute(string? relativePath, string workspaceRoot) {
        if (string.IsNullOrWhiteSpace(relativePath) || string.IsNullOrWhiteSpace(workspaceRoot)) { return null; }
        ...
        // 检查是否越界
        if (!IsWithinWorkspace(relativePath, workspaceRoot)) { return null; }
