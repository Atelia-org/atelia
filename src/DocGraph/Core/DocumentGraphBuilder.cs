...
public partial class DocumentGraphBuilder : IDocumentGraphBuilder
{
    ...
    public DocumentGraph Build(IEnumerable<string>? wishDirectories = null)
    {
        ...
        foreach (var dir in directories)
        {
            ...
            foreach (var filePath in mdFiles)
            {
                ...
                // 尝试解析 frontmatter
                if (!FrontmatterParser.TryParseFile(filePath, out var frontmatter, out _) || frontmatter == null)
                {
                    continue; // 跳过无 frontmatter 的文件
                }
        ...
        List<IFixAction> fixActions)
    {
        foreach (var targetPath in node.ProducePaths)
        {
            ...
            if (!graph.TryGetNode(normalizedPath, out var targetNode) || targetNode == null)
            {
                continue; // 节点不在图中（不应发生，但防御性处理）
            }
