...
public sealed class RbfScanner : IRbfScanner {
    ...
    private void ScanReverseInternal(List<RbfFrame> results) {
        ...
        if (fileLength < RbfConstants.FenceLength) { return; }
    ...
    private static bool IsFenceAt(ReadOnlySpan<byte> span, long offset) {
        if (offset < 0 || offset + RbfConstants.FenceLength > span.Length) { return false; }
    ...
    private bool TryReadFrameAt(long frameStart, out RbfFrame frame) {
        ...
        // 检查前置 Fence
        long prevFencePos = frameStart - RbfConstants.FenceLength;
        if (prevFencePos < 0 || !IsFenceAt(span, prevFencePos)) { return false; }

        // 读取 HeadLen
        if (frameStart + 4 > span.Length) { return false; }
        ...
        if (recordEnd + RbfConstants.FenceLength > span.Length) { return false; }

        // 检查尾部 Fence
        if (!IsFenceAt(span, recordEnd)) { return false; }
    ...
    private static bool TryValidateFrame(ReadOnlySpan<byte> span, long frameStart, uint headLen, uint storedCrc, out RbfFrame frame) {
        ...
        // 确保有足够的空间读取
        if (recordEnd - 9 < frameStart + 8) { return false; }
        ...
        for (int i = 0; i < statusLen; i++) {
            if (statusBytes[i] != statusByte) { return false; }
        }
