...
### spec [S-MEMOTREE-SPLIT-MERGE-PRIMITIVES] 长节点治理应建立在 split/merge 原语之上

对于“同一节点同时拥有较长正文和较多子节点”的情况，MemoTree SHOULD 优先提供显式的 `split` / `merge` 原语，而不是先发明高层自动重组策略。

推荐的 v0 原语包括：

- 文本层：把一个正文块切成两个相邻 block，或把相邻 block 合并
- 节点层：把一个节点切成两个相邻 sibling，或把一个节点与它的下一个 sibling 合并

这样做的原因是：

- 这些操作可审计、可组合、可测试
- LLM 可以先用原语完成结构整理，再逐步形成更高层习惯
- 先提供能力，比一开始就预判“什么时候该拆”更稳妥

### spec [S-MEMOTREE-NODE-SPLIT-ON-STRUCTURAL-BOUNDARIES] 节点切分只认结构边界

MemoTree 的节点层 `SplitNode` SHOULD 只接受结构边界，不应直接暴露 char offset。

当前推荐的结构边界有两类：

- `AfterBodyBlockId`：在某个正文块之后切
- `BeforeChildId`：在某个子节点之前切
...
这与 `DurableText.SplitBlock`“左侧保留原 ID、右侧拿新 ID”的稳定引用策略保持一致。

### spec [S-MEMOTREE-TEXT-SPLIT-HELPER-FOR-LLM] 面向 LLM 的切分点描述应放在文本层 helper

对 LLM 来说，char offset 太不自然。更适合的方式是提供一个文本层 helper，让调用方用“前后文本”描述切分点，再把它稳定化为新的 block 边界。

推荐思路：

- 输入 `blockId`
- 输入 `AfterText` / `BeforeText`
- helper 在该 block 内定位切分点
- helper 调用底层 `DurableText.SplitBlock(...)`
- 返回新的左右 `blockId`
- 之后节点层 `SplitNode` 再基于这些 block 边界做结构切分

这意味着：

- 文本层 helper 负责把“模糊的人类描述”变成“稳定的块边界”
- 节点层原语负责真正改变 MemoTree 结构

这种两层分工比把文本定位和结构变更硬塞进一个大操作里更清晰。

### spec [S-MEMOTREE-MERGE-LEFT-KEEPS-IDENTITY] MergeWithNextSibling 由左节点保留身份

当前推荐的节点合并语义是：

- `MergeNodeWithNextSibling(nodeId)`
- 当前节点保留原 `nodeId`
- 下一个 sibling 被吸收并删除
- `body` 顺序拼接
- `children` 顺序拼接
- `tags` 默认并集
- `gist/summary` 默认视为需要重算

也就是说，`split` 与 `merge` 在身份保持策略上都偏向“左侧保留、右侧新建/删除”，这样更有利于稳定引用。
...
`SplitNode` / `MergeNodeWithNextSibling` 这类结构重组操作 SHOULD 默认使相关节点的 `gist` / `summary` 失效，除非调用方显式提交新的派生记忆。
...
如果后续把 split/merge 暴露给 Agent，推荐的工具分层是：

- 文本层 helper：基于 `AfterText` / `BeforeText` 建立 block 边界
- 节点层原语：基于 block/child 边界做 `SplitNode` / `MergeNodeWithNextSibling`
...
3. 再补 `IApp` 适配、`index+flatten` Window 投影、`Collapse` 记忆维护工具、split/merge 原语与最小工具集
