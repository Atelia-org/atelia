...
> 状态：按 2026-06-01 当前代码与测试基线修订（含 F9 第一拍、F10 第一拍、T1 第三拍）
...
- cross-host parity 第一拍已完成：
  ...
  - 当前已 guard 的 canonical seam 包括：
    ...
    - minimal authoring snapshot
      - create location
      - create actor
      - create passage（默认 `travelMode/baseTravelCost`）
      - create passage（显式 `travelMode + baseTravelCost`）
...
- 最近一次全量基线曾为通过；本拍实际回归的是 `CrossHostMachineContractParityTests` 过滤器
...
`F10` 已经把 host failure contract 收住，`T1` 也已经进一步把剩余主入口收窄到更少数的 non-canonical runtime seam 与 host authoring parity tail。
...
- 如后续需要，再继续扩更细的 host authoring parity tail（例如超出当前 success snapshot 基线的 request 语义）
...
- `create-passage` request 语义
  - 当前 success-path snapshot 已守住默认值与显式 `travelMode/baseTravelCost`
  - 但 host JSON body 与 CLI positional 参数的更广义 authoring 语义差异仍应保留为 residual risk，而不是在这一拍硬收成统一工程

因此，后续最自然的连续实施对象已经不再是“继续补主入口”，而是少数更难的 non-canonical runtime seam，或零散的 host authoring parity tail。
...
- 后续 `T1` 已继续把下面这些 seam 补进 parity suite：
  ...
  - create passage（默认 `travelMode/baseTravelCost`）
  - create passage（显式 `travelMode + baseTravelCost`）
- `create location` / `create actor` / `create passage` 的 GameServer request body 现在都已显式写成原始 JSON，而不是依赖测试客户端 serializer 隐式生成
- 已新增单独的 canonical machine surface 文档，不再把稳定 contract 承诺埋在 `refactor-plan` 里

当前残余风险：

- `create-passage` 的 host JSON request 与 CLI positional 参数语义仍未做全面统一
- 当前 parity suite 只守住 empty-repo authoring workflow 下的成功快照，不包含 invalid input / error envelope / 更宽口径参数组合
...
- 下一自然入口：
  ...
  - 或继续处理更细的 host authoring parity tail，但不要把 `create-passage` request 语义差异误表述成已经统一

当前判断：

- 这部分已经把 T1 的主入口进一步收窄
- 当前下一拍更自然的方向，已经更多转向更难的 non-canonical runtime seam，或更零散的 host authoring parity tail
