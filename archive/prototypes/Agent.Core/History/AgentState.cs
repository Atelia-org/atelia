...
memory_notebook_replace与memory_notebook_replace_span工具就是为你主动编辑你自己的[Memory Notebook]而设计的。";
    ...
    internal bool FoldPendingNotificationsIntoCurrentToolResultsForPendingModelCall() {
        EnsureWorkspaceSessionOpen();
        if (!HasPendingNotification) {
            return false;
        }
        ...
        if (RecentHistory.Count == 0 || RecentHistory[^1] is not ToolResultsEntry toolResultsEntry) {
            throw new InvalidOperationException("Cannot fold pending notifications because the current tail is not a ToolResultsEntry.");
        }
        ...
        _workingSet.ReplaceLastHistoryEntry(ObservationEntryMutationHelper.CloneWithMergedNotifications(toolResultsEntry, notifications));
        return true;
    }
