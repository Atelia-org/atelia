...
/// 职能：
/// - 摄取 (Ingest): 接收即将被 Dequeue 的消息轮次。
/// - 精炼 (Refine): 提取消息中的关键信息，摒弃冗余和过时的细节。
/// - 融合 (Merge): 将精炼后的内容更新到现有的 Recap 文本中，确保连贯性和可读性。
/// - 修剪 (Prune): 当 Recap 本身过长时，进一步遗忘后效性较低的信息，控制总体长度。
/// 工作方式 (设计初稿):
...
/// - 主动记忆管理：Agent 自主判断何时需要更新 Recap。
/// - 外部记忆集成：将 Recap 文本作为检索增强生成 (RAG) 的索引源。
/// - 分层记忆架构：短期记忆（近期消息）→ 中期记忆（Recap）→ 长期记忆（外部存储）。
