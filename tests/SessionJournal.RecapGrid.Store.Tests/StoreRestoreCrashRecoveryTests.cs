...
public sealed class StoreRestoreCrashRecoveryTests : IDisposable {
    private readonly string _root = Path.Combine(
        Path.GetTempPath(),
        "atelia-recap-grid-v4-restore-crash-tests",
    ...
    public void V4RestoreCrashHasExactPhaseOutcomeAndCanUpgradeAgain(
        bool partial,
        string failpoint
    ) {
        ...
        V4UpgradeCrashFixture fixture = V4UpgradeCrashFixture.Create(
            repository, partial);
        Func<IReadOnlyList<RowWork>> resolver = partial
            ? () => [fixture.ExpectedWork]
            : static () => [];
        RecapGridStoreUpgradeResult.Upgraded upgraded = Assert.IsType<
            ...
            RecapGridStoreMaintenance.UpgradeV4(
                repository, apply: true, resolver));
        Assert.Equal(fixture.OriginalDatabase,
            File.ReadAllBytes(upgraded.BackupPath));
        byte[] originalV5 = File.ReadAllBytes(fixture.DatabasePath);
        RecapGridStorePrepareRestoreResult.Prepared prepared = Assert.IsType<
            ...
            RecapGridStoreMaintenance.PrepareRestoreV4(
                repository, upgraded.BackupPath, resolver));
        ...
        RunCrash(failpoint, repository, upgraded.BackupPath,
            prepared.Active.Witness, prepared.Backup.Witness, proofBundle);

        AssertNoActiveSidecars(fixture.DatabasePath);
        AssertOutsideUnchanged(repository, outsideBefore);
        Assert.Equal(fixture.OriginalDatabase,
            File.ReadAllBytes(upgraded.BackupPath));
        AssertStrictV4(upgraded.BackupPath);
        string[] orphanTemporaries = Directory.GetFiles(
            new StorePaths(repository).RootPath,
            ".grid.restore-v4.*.sqlite");
        bool preReplace = failpoint == "after-temp-verified";
        if (preReplace) {
            Assert.Equal(originalV5, File.ReadAllBytes(fixture.DatabasePath));
            ...
            Assert.Equal(fixture.OriginalDatabase,
                File.ReadAllBytes(orphan));
            AssertStrictV4(orphan);

            Assert.IsType<RecapGridStoreRestoreResult.Restored>(
                RecapGridStoreMaintenance.RestoreV4(
                    repository, upgraded.BackupPath,
                    prepared.Active.Witness, prepared.Backup.Witness,
                    resolver));
            ...
            RecapGridStoreReaderOpenResult.UnsupportedSchema unsupported =
                Assert.IsType<
                    RecapGridStoreReaderOpenResult.UnsupportedSchema>(
                    RecapGridStoreFactory.OpenReader(repository));
            Assert.Equal(4, unsupported.SchemaVersion);
    ...
    private static void RunCrash(
        ...
        start.ArgumentList.Add("restore-v4");
        start.ArgumentList.Add(failpoint);
        start.ArgumentList.Add(repository);
        start.ArgumentList.Add(backup);
        start.ArgumentList.Add(activeWitness.Length.ToString(
            System.Globalization.CultureInfo.InvariantCulture));
        start.ArgumentList.Add(activeWitness.Sha256);
        start.ArgumentList.Add(backupWitness.Length.ToString(
            System.Globalization.CultureInfo.InvariantCulture));
        start.ArgumentList.Add(backupWitness.Sha256);
        ...
        Assert.True(standardError.Contains($"restore-v4/{failpoint}",
