...
public class DurableDictTests {
    ...
    public void UnmodifiedKey_ReadsFromCommitted() {
        ...
        // Assert
        dict[1].Should().Be("ONE");  // 从 _current
        dict[2].Should().Be("two");  // 从 _current（未修改）
    }
    ...
    public void TryGetValue_ObjectId_TriggersLazyLoad() {
        // Arrange: 模拟 Workspace 和 ObjectLoader
        var targetObjectId = 100UL;
        var referencedDict = new DurableDict(targetObjectId);  // 被引用的对象
    ...
    public void Indexer_ObjectId_TriggersLazyLoad() {
        ...
        var referencedDict = new DurableDict(targetObjectId);
