...
public class IncrementalProcessorInMemoryTests {
    ...
    public void Processor_SkipsOutlineWrite_WhenStructureHashUnchanged() {
        ...
        // 第一次写入
        var res1 = proc.Process(idx, impact, hasher, outline, Resolver, @"C:\proj\outlines", default, null, new InMemoryFsAdapter(fs));
        ...
        var res2 = proc.Process(idx, impact, hasher, outline, Resolver, @"C:\proj\outlines", default, null, new InMemoryFsAdapter(fs));
    ...
    public void Processor_UpdatesManifest_AndOutline_InMemory() {
        ...
        INamedTypeSymbol? Resolver(string id) => sym;
        var proc = new IncrementalProcessor();
        var res = proc.Process(idx, impact, hasher, outline, Resolver, @"C:\proj\outlines", default, null, new InMemoryFsAdapter(fs));
