...
public sealed class GalateaConfigValidationTests {
    ...
    public void HomeDirectoryMustBeExistingCanonicalAllowedAndDisjoint(string scenario) {
        ...
        try {
            ...
            JsonObject user = config["characters"]!.AsArray()[0]!.AsObject();
    ...
    public void UserHomesCannotBeSharedOrNested(bool nested) {
        ...
        try {
            ...
            Assert.NotEqual(valid.Characters[0].HomeDir, valid.Characters[1].HomeDir);
            JsonObject config = JsonNode.Parse(File.ReadAllText(configPath))!.AsObject();
            string otherHome = nested
                ? Directory.CreateDirectory(Path.Combine(valid.Characters[0].HomeDir, "bob")).FullName
                : valid.Characters[0].HomeDir;
            config["characters"]!.AsArray()[1]!["homeDir"] = otherHome;
            File.WriteAllText(configPath, config.ToJsonString());
    ...
    [Fact]
    public async Task AutonomyEnrollmentIsSnapshottedForRuntimeAndHost() {
        string root = NewRoot();
        try {
            string configPath = WriteConfig(root,
                [User("alice", Path.Combine(root, "sessions", "alice"))]);
            GalateaConfig loaded = GalateaConfigLoader.Load(configPath);
            var characters = loaded.Characters.Select(character => character with { AutonomyIntervalMinutes = 10 }).ToList();
            ...
            characters[0] = characters[0] with { AutonomyIntervalMinutes = 0 };
            Assert.Equal(["alice"], service.AutonomyCharacterIds);
            Assert.Throws<NotSupportedException>(() =>
                ((IList<string>)service.AutonomyCharacterIds)[0] = "changed");
            Assert.Empty(initializerConfig.AutonomyCharacterIds);
    ...
    public void FileConfigDtosAreInternalWhileRuntimeAndHttpDtosRemainPublic() {
        ...
        Assert.False(typeof(GalateaRootFileConfig).IsPublic);
        Assert.False(typeof(GalateaCharacterFileConfig).IsPublic);
        Assert.False(typeof(GalateaRecapGridFileConfig).IsPublic);
        Assert.DoesNotContain(typeof(GalateaRootFileConfig), exported);
        Assert.DoesNotContain(typeof(GalateaCharacterFileConfig), exported);
        Assert.DoesNotContain(typeof(GalateaRecapGridFileConfig), exported);

        Assert.Contains(typeof(GalateaConfig), exported);
        Assert.Contains(typeof(GalateaCharacterConfig), exported);
    ...
    public void RootConfigTemplateStartsWithExactV14AndRoundTrips() {
        ...
        GalateaStrictConfigReader.ValidateRoot(template);
        ...
        Assert.All(decoded.Characters, static character => Assert.Equal(0, character.AutonomyIntervalMinutes));
        Assert.Equal(["/galatea-homes/alice", "/galatea-homes/bob"],
            decoded.Characters.Select(static user => user.HomeDir));
        Assert.Equal(
        ...
        Assert.Equal(
            ...
            decoded.Characters.Select(static user => user.SessionDir)
        );
        Assert.Equal(
            ["delegation-state/alice", "delegation-state/bob"],
            decoded.Characters.Select(static user => user.DelegationStateDir)
        );
        Assert.Equal(
            ["character-memory/alice", "character-memory/bob"],
            decoded.Characters.Select(static user =>
                user.CharacterMemoryStateDir)
        );
        Assert.Equal(
            ["Alice", "Bob"],
            decoded.Characters.Select(static user => user.CharacterName)
        );
        Assert.Equal(
            ["玩家"],
            decoded.Players.Select(static player => player.Name)
        );
        Assert.All(decoded.Characters, static user => Assert.Equal(
            string.Empty,
    ...
    public void ProductionBootstrapWritesBomlessRootConfigThatStrictLoaderReads() {
        ...
        try {
            ...
            GalateaStrictConfigReader.ValidateRoot(usersBytes);
            ...
            foreach (JsonNode? value in configured["characters"]!.AsArray()) {
                JsonObject user = value!.AsObject();
                user["homeDir"] = Directory.CreateDirectory(Path.Combine(
                    root, "homes", user["id"]!.GetValue<string>())).FullName;
            }
            ...
            Assert.Equal(
                ["Alice", "Bob"],
                loaded.Characters.Select(static user =>
                    user.CharacterName.Value)
            );
            Assert.Equal(
                ["玩家"],
                loaded.Players.Select(static player => player.Name.Value)
            );
            ...
            Assert.All(loaded.Characters, static user => Assert.Equal(
                GalateaConfigTemplateFactory.DefaultConnectionId,
                user.DefaultConnectionId
            ));
            Assert.Equal(
                ...
                loaded.Characters.Select(static user => user.SessionDir)
            );
            Assert.Equal(
                [
    ...
    public void ExistingConfigBootstrapCreatesMissingInRootPromptOnce() {
        ...
        try {
            ...
            GalateaCharacterConfig user = Assert.Single(
                GalateaConfigLoader.Load(configPath).Characters
            );
            Assert.True(user.SystemPrompt.IsStructured);
            string projected = GalateaInputProjector.Instance.Project(user.SystemPrompt);
            Assert.Contains("Galatea", projected,
                StringComparison.Ordinal);
            Assert.DoesNotContain("刘世超", projected,
                StringComparison.Ordinal);
            Assert.Contains("GM carrier", projected,
                StringComparison.Ordinal);
            Assert.Contains("## 界外邮箱", projected,
                StringComparison.Ordinal);
            Assert.DoesNotContain("### 发信给 Codex", projected,
                StringComparison.Ordinal);
            Assert.DoesNotContain(
                "### 保存长期 Note",
                projected,
        ...
        try {
            ...
            File.WriteAllText(
                ...
                JsonSerializer.Serialize(
                    GalateaConfigTemplateFactory.CreateRootFile() with {
    ...
    public async Task RelativeSessionDirectoryUsesConfigDirectoryNotProcessCurrentDirectory() {
        ...
        try {
            ...
            GalateaConfig loaded = GalateaConfigLoader.Load(configPath);
            GalateaCharacterConfig loadedUser = Assert.Single(loaded.Characters);
    ...
    public void AbsoluteSessionDirectoryTargetIsPreserved() {
        ...
        try {
            ...
            Assert.Equal(
                ...
                Assert.Single(loaded.Characters).SessionDir
            );
    ...
    public void DelegationStatePathsResolveExactlyAndMustRemainDisjoint() {
        ...
        try {
            ...
            Assert.Equal(
                absolute,
                Assert.Single(GalateaConfigLoader.Load(configPath).Characters)
                    .DelegationStateDir
            );

            GalateaCharacterConfig[] duplicate = [
            ...
            GalateaCharacterConfig[] nestedDelegation = [
            ...
            GalateaCharacterConfig[] crossUserNesting = [
                User(
                    "alice",
                    "sessions/alice",
    ...
    public void CharacterMemoryStatePathsResolveExactlyAndRemainDisjoint() {
        ...
        try {
            ...
            GalateaCharacterConfig loaded = Assert.Single(
                GalateaConfigLoader.Load(configPath).Characters
            );
    ...
    public void DirectConfigConstructionUsesTotalStorageTopology() {
        ...
        try {
            ...
            GalateaCharacterConfig[] duplicateCharacterMemory = [
                User(
                    "alice",
                    Path.Combine(root, "sessions", "alice"),
            ...
            var duplicateConfig = new GalateaConfig(
                ...
                [new GalateaPlayerConfig("player-main", new GalateaPlayerName("刘世超"), "pw")],
                Connections,
    ...
    public void DirectConfigConstructionRejectsNonCanonicalStoragePaths(
        ...
        string root = NewRoot();
        try {
            GalateaCharacterConfig baseline = User(
                "alice",
                Path.Combine(root, "sessions", "alice")
            );
            ...
            foreach (string invalidPath in invalidPaths) {
                GalateaCharacterConfig invalidUser = storageField switch {
    ...
    public void RootConfigAcceptsExactV14OutsideFirstProperty() {
        ...
        try {
            ...
            GalateaConfig loaded = GalateaConfigLoader.Load(configPath);
            Assert.Equal("alice", Assert.Single(loaded.Characters).CharacterId);
        }
        finally {
            Directory.Delete(root, recursive: true);
        }
    ...
    public void BootstrapDoesNotRewriteExistingVersionlessConfig() {
        ...
        try {
            ...
            using (JsonDocument document = JsonDocument.Parse(versionless)) {
                Assert.False(document.RootElement.TryGetProperty("v", out _));
            }
    ...
    public async Task StrictRecapGridConfigDefersRouteReadAndCompletionClientCreation() {
        ...
        try {
            ...
            Assert.Equal(
                "test",
                Assert.Single(loaded.Characters).DefaultConnectionId
            );
            Assert.Single(service.ConnectionsFor("alice"));
        ...
        try {
            ...
            File.WriteAllText(
                ...
                JsonSerializer.Serialize(
                    ...
                    Players: [new GalateaPlayerFileConfig("player-main", "刘世超", "pw")],
    ...
    [Fact]
    public void ConnectionsRequireCompletionOwnedV3AndBootstrapRoundTrips() {
        byte[] template = GalateaConfigTemplateFactory
            .CreateConnectionsFileUtf8();
        CompletionConnectionCatalogConfig decoded =
            CompletionConnectionConfigLoader.DecodeCatalog(template);
        Assert.Single(decoded.Connections);
        Assert.Null(decoded.SelectableConnectionIds);
        ...
        using (JsonDocument document = JsonDocument.Parse(template)) {
            ...
            Assert.False(root.TryGetProperty("defaultConnectionId", out _));
        ...
        JsonObject withGlobalDefault = JsonNode.Parse(template)!.AsObject();
        withGlobalDefault["defaultConnectionId"] =
            GalateaConfigTemplateFactory.DefaultConnectionId;
        Assert.Throws<InvalidDataException>(() =>
            CompletionConnectionConfigLoader.DecodeCatalog(
                Encoding.UTF8.GetBytes(withGlobalDefault.ToJsonString())
            )
        );
    ...
    [Fact]
    public async Task PerCharacterOptionsConstrainFreshSelectionAndHtml() {
        string root = NewRoot();
        try {
            CompletionConnectionConfig first = Connections[0] with {
                Id = "first",
                ModelId = "model-first"
            };
            CompletionConnectionConfig second = Connections[0] with {
                Id = "second",
            ...
            string configPath = WriteConfig(
                ...
                [
                    User(
                        "alice",
                        Path.Combine(root, "alice-session"),
                        defaultConnectionId: first.Id
                    ) with { ConnectionOptions = [new(first.Id, "living", "") ] },
                    User(
                        "bob",
                        Path.Combine(root, "bob-session"),
                        defaultConnectionId: second.Id
                    ) with { ConnectionOptions = [new(second.Id, "working", "") ] }
                ],
            ...
            Assert.Equal(
                [first.Id, second.Id],
                loaded.Characters.Select(static user => user.DefaultConnectionId)
            );
            await using var service = new GalateaHostService(
                loaded,
                new TrackingFactory(),
            ...
            Assert.Equal([first.Id], service.ConnectionsFor("alice").Select(static option => option.Id));
            Assert.Equal([second.Id], service.ConnectionsFor("bob").Select(static option => option.Id));
            Assert.False(service.TryGetFreshConnection(loaded.Characters[0], second.Id, out _));
            Assert.False(service.TryGetRecoveryConnection(loaded.Characters[1], first.Id, out _));
            Assert.Throws<ArgumentException>(() => service.SetRuntimeConnectionOverride("alice", second.Id));
            string aliceHtml = GalateaHtml.RenderAppPage(
                loaded.Characters[0],
                loaded.Players[0],
                service.ConnectionsFor("alice"),
                maintenanceMode: false,
                assetVersion: "test"
            );
            string bobHtml = GalateaHtml.RenderAppPage(
                loaded.Characters[1],
                loaded.Players[0],
                service.ConnectionsFor("bob"),
                maintenanceMode: false,
                assetVersion: "test"
            );
            Assert.DoesNotContain("defaultConnectionId:", aliceHtml);
            Assert.DoesNotContain("defaultConnectionId:", bobHtml);
            Assert.Contains("model-first", aliceHtml);
            Assert.DoesNotContain("model-second", aliceHtml);
            Assert.Contains("model-second", bobHtml);
            Assert.DoesNotContain("model-first", bobHtml);
        }
        finally {
            Directory.Delete(root, recursive: true);
        }
    ...
    [Fact]
    public void CharacterConnectionOptionsRequireExactCatalogIdsAndMetadataStrings() {
        string root = NewRoot();
        try {
            CompletionConnectionConfig other = Connections[0] with { Id = "other" };
            string configPath = WriteConfig(root,
                [User("alice", Path.Combine(root, "session"))],
                connections: [.. Connections, other]);
            JsonObject valid = JsonNode.Parse(File.ReadAllText(configPath))!.AsObject();
            JsonObject character = valid["characters"]![0]!.AsObject();
            JsonObject option = character["connectionOptions"]![0]!.AsObject();
            option["name"] = "生活状态";
            option["trigger"] = "穿着裙装";
            File.WriteAllText(configPath, valid.ToJsonString());
            GalateaCharacterConnectionOption loaded = Assert.Single(
                Assert.Single(GalateaConfigLoader.Load(configPath).Characters).ConnectionOptions);
            Assert.Equal("生活状态", loaded.Name);
            Assert.Equal("穿着裙装", loaded.Trigger);

            void Reject(Action<JsonObject> change) {
                JsonObject invalid = valid.DeepClone().AsObject();
                change(invalid["characters"]![0]!.AsObject());
                File.WriteAllText(configPath, invalid.ToJsonString());
                Assert.ThrowsAny<Exception>(() => GalateaConfigLoader.Load(configPath));
            }

            Reject(static ch => ch.Remove("connectionOptions"));
            Reject(static ch => ch["connectionOptions"] = new JsonArray());
            Reject(static ch => ch["connectionOptions"]![0]!["connectionId"] = "missing");
            Reject(static ch => ch["connectionOptions"]![0]!["connectionId"] = "other");
            Reject(static ch => ch["connectionOptions"]!.AsArray().Add(
                ch["connectionOptions"]![0]!.DeepClone()));
            Reject(static ch => ch["connectionOptions"]![0]!.AsObject().Remove("name"));
            Reject(static ch => ch["connectionOptions"]![0]!["trigger"] = null);
        }
        finally {
            Directory.Delete(root, recursive: true);
        }
    ...
    [InlineData("missing")]
    [InlineData("hidden")]
    public void PerUserDefaultMustBeBoundedExactAndSelectable(
        string defaultConnectionId
    ) {
        string root = NewRoot();
        try {
            CompletionConnectionConfig hidden = Connections[0] with {
                Id = "hidden"
            };
            string configPath = WriteConfig(
                root,
    ...
    [Fact]
    public void PerUserDefaultRejectsOverBoundIdentifier() {
        string root = NewRoot();
        try {
            string configPath = WriteConfig(
                root,
                [User(
                    "alice",
                    Path.Combine(root, "session"),
                    defaultConnectionId: new string('a', 129)
                )]
            );
    ...
    public void GalateaConnectionsRequireExactSelectionAndFeatureBindings() {
        ...
        try {
            ...
            string disabledPrompt = GalateaInputProjector.Instance.Project(Assert.Single(
                outboundDisabled.Characters
            ).SystemPrompt);
            ...
            string enabledPrompt = GalateaInputProjector.Instance.Project(Assert.Single(
                outboundEnabled.Characters
            ).SystemPrompt);
            Assert.Equal(
                "test",
            ...
            Assert.Contains(Assert.Single(outboundEnabled.Characters).HomeDir, enabledPrompt,
                StringComparison.Ordinal);
            ...
            string noteEnabledPrompt = GalateaInputProjector.Instance.Project(Assert.Single(
                noteEnabled.Characters
            ).SystemPrompt);
            ...
            string bothEnabledPrompt = GalateaInputProjector.Instance.Project(Assert.Single(
                Load(bothEnabledJson).Characters
            ).SystemPrompt);
            Assert.True(
            ...
            JsonObject staleGlobalSelection = original.DeepClone().AsObject();
            staleGlobalSelection["selectableConnectionIds"] = new JsonArray("test");
            Assert.Throws<InvalidDataException>(() => Load(staleGlobalSelection));
    ...
    private static void AssertRejectedByLoaderAndHost(
        ...
        if (expectedNonCanonicalHostUserId is null) {
            ...
            Assert.Contains(
                $"sessionDir for character '{expectedNonCanonicalHostUserId}'",
    ...
    private static string WriteConfig(
        ...
        File.WriteAllText(
            ...
            JsonSerializer.Serialize(
                new GalateaRootFileConfig(
                    Version: GalateaStrictConfigReader.CurrentConfigVersion,
                    Characters: users.Select(user => FileUser(user) with {
                        HomeDir = Directory.CreateDirectory(Path.Combine(root, "homes", user.CharacterId)).FullName,
                        ConnectionOptions = connectionOptionIds is null
                            ? user.ConnectionOptions
                            : connectionOptionIds.Select(static id =>
                                new GalateaCharacterConnectionOption(id, "", "")).ToArray()
                    }).ToArray(),
                    Players: [new GalateaPlayerFileConfig("player-main", "刘世超", "pw")],
                    Runtime: new GalateaRuntimeFileConfig(
    ...
    private static string WriteFileConfig(
        string root,
        IReadOnlyList<GalateaCharacterFileConfig> users
    ) {
        string configPath = Path.Combine(root, "config.json");
        File.WriteAllText(
            configPath,
            JsonSerializer.Serialize(
                new GalateaRootFileConfig(
                    Version: GalateaStrictConfigReader.CurrentConfigVersion,
                    Characters: users.Select(user => user with {
                        HomeDir = Directory.CreateDirectory(Path.Combine(root, "homes", user.CharacterId)).FullName
                    }).ToArray(),
                    Players: [new GalateaPlayerFileConfig("player-main", "刘世超", "pw")],
                    Runtime: new GalateaRuntimeFileConfig(
    ...
    private static GalateaCharacterFileConfig FileUser(
    ...
    private static GalateaCharacterConfig User(
        ...
        userId,
        new GalateaCharacterName("Character-" + userId),
        sessionDirectory,
        ...
        GalateaSessionProvisioning.ExistingOnly,
        SystemPrompt: "prompt Character-" + userId,
        DefaultConnectionId: defaultConnectionId,
        ConnectionOptions: [new("test", "", "")]
    );
