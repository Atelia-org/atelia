...
- Galatea dispatch首次检查、`OutcomeUnknown`、terminal与任何runtime identity不一致都使用`thread/read(includeTurns=true)`重新核对完整history中的exact dispatch ID、task body与turn ID。只有同一app-server generation内的full inspection和runtime通知共同确认该turn仍为running后，warm process才缓存这一份非durable证明；后续轮询仍以`thread/read(includeTurns=false)`重新验证thread ID、ownership name、canonical cwd和active状态，并在请求前后都要求runtime `activeTurnId`精确匹配。缓存显式限制为至多32个thread、每个thread当前一个dispatch且task不超过128 KiB UTF-8；超限只退化为full inspection。terminal/new-turn通知、app-server退出和Bridge停止都会即时清理对应证明。cold/process restart后不从persisted `inProgress`重建live cache：真实的无执行者turn应由app-server映射为Interrupted；若上游仍暂报Running，则保守地持续full inspection。缓存不是恢复或业务authority。
