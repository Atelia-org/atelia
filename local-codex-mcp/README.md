...
- Galatea dispatch首次检查、`OutcomeUnknown`、cold restart、terminal与任何runtime identity不一致都使用`thread/read(includeTurns=true)`重新核对完整history中的exact dispatch ID、task body与turn ID。只有full inspection已确认同一turn仍为running后，warm process才缓存这一份非durable证明；后续轮询仍以`thread/read(includeTurns=false)`重新验证thread ID、ownership name、canonical cwd和active状态，并同时要求runtime `activeTurnId`精确匹配。每个thread至多缓存当前一个dispatch，terminal、app-server退出和Bridge停止都会清理；缓存不是恢复或业务authority。
