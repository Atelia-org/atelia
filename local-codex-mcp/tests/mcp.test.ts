...
import type { TaskBackend, TaskSnapshot } from "../src/backend/task-backend.js";
...
class StubBackend implements TaskBackend {
  ...
  async start() {}
  async stop() {}
  async delegate(input: { cwd?: string }) {
    this.calls += 1;
...
test("tools/list exposes the five bounded tools with accurate annotations", async (t) => {
  ...
  assert.equal(tools.tools.find((tool) => tool.name === "codex_delegate")?.annotations?.readOnlyHint, false);
});
